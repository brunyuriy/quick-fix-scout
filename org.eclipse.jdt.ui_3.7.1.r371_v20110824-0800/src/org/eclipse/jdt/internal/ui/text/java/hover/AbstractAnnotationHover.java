...
public abstract class AbstractAnnotationHover extends AbstractJavaEditorTextHover {
	...
	// I need to access this class, so I made it public.
//	private static class AnnotationInformationControl extends AbstractInformationControl implements IInformationControlExtension2 {
    public static class AnnotationInformationControl extends AbstractInformationControl implements IInformationControlExtension2 {
    // XXX Kivanc ends here
	    ...
	    {
	        // I turned to lazy initialization. (Probably to make sure that requested object is created by that time)
	    ...
	    {
	        return speculationHack_ != null && proposals_ == null;
	    }
	    
	    private boolean shallCallObserver()
	    {
	        return observerHack_ != null && proposals_ == null;
	    }
	    
	    private boolean isObserverSet()
	    {
	        return observerHack_ != null;
	    }
	    
	    private boolean isSpeculatorSet()
	    {
	        return speculationHack_ != null;
	    }
	    
	    private String [] displayStrings_;
	    private ICompletionProposal [] proposals_;
	    
	    public void setProposals(ICompletionProposal [] proposals, String [] displayStrings)
	    {
	        proposals_ = proposals;
	        displayStrings_ = displayStrings;
	        // The two below recreates the information in the hover dialog.
            disposeDeferredCreatedContent();
            deferredCreateContent();
	    }
	    
	    private String getDisplayString(ICompletionProposal proposal, int index)
	    {
	        if (displayStrings_ == null)
	            return proposal.getDisplayString();
	        if (index >= displayStrings_.length)
	            return proposal.getDisplayString();
	        return displayStrings_[index];
	    }
	    // XXX Kivanc ends here...
		...
		public AnnotationInformationControl(Shell parentShell, ToolBarManager toolBarManager) {
           ...
           if (shallCallObserver())
               observerHack_.hoverPopupCreated();
           if (shallCallSpeculator())
               speculationHack_.hoverPopupCreated(this);
           // XXX Kivanc ends here
		}
		...
		protected void disposeDeferredCreatedContent() {
			...
			// XXX Kivanc stats here.
			if (proposals_ == null)
			{
			    if (shallCallObserver())
			        observerHack_.hoverPopupClosed();
			    if (shallCallSpeculator())
...
//			ICompletionProposal[] proposals= getAnnotationInfo().getCompletionProposals();
			ICompletionProposal [] proposals = proposals_;
			if (proposals == null)
			{
                proposals = getAnnotationInfo().getCompletionProposals();
                if (shallCallSpeculator())
                    speculationHack_.hoverProposalsSet(proposals);
			}
			// XXX Kivanc ends here.
		...
		private void createCompletionProposalsList(Composite parent, ICompletionProposal[] proposals) {
			...
			for (int i= 0; i < proposals.length; i++) {
			    // XXX Kivanc starts here
				list.add(createCompletionProposalLink(composite, proposals[i], getDisplayString(proposals[i], i), 1));// Original link for single fix, hence pass 1 for count
				// XXX Kivanc ends here
				...
				if (proposals[i] instanceof FixCorrectionProposal) {
					...
					if (count > 1) {
					    // XXX Kivanc starts here
						list.add(createCompletionProposalLink(composite, proposals[i], getDisplayString(proposal, i), count));
						// XXX Kivanc ends here
					}
				}
			}
		...
		// XXX Kivanc starts here.
		// I commented the following method because I need to access it and there is no easy way to do it. So, I made it
		// public.
//		private Link createCompletionProposalLink(Composite parent, final ICompletionProposal proposal, int count) {
		public Link createCompletionProposalLink(Composite parent, final ICompletionProposal proposal, String displayString, int count) {
		// XXX Kivanc ends here
			...
			if (isMultiFix) {
			    ...
			    // XXX Kivanc starts here.
			    // The following is overwritten so that my display string will be written on the hover.
//				linkText= proposal.getDisplayString();
			    linkText = displayString;
				// XXX Kivanc ends here.
			}
		...
		private void apply(ICompletionProposal p, ITextViewer viewer, int offset, boolean isMultiFix) {
            ...
            System.out.println("proposals = " + proposals_);
            // XXX Kivanc starts here.
            if (isObserverSet())
