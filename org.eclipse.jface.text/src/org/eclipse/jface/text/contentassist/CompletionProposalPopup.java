...
class CompletionProposalPopup implements IContentAssistListener {
    ...
    private boolean isQFCall()
    {
        if (fContentAssistant == null)
            return false;
        return fContentAssistant.getClass().getName().equals("org.eclipse.jface.text.quickassist.QuickAssistAssistant$QuickAssistAssistantImpl");
    }
    
    private boolean shallCallSpeculator()
    {
        return speculationHack_ != null && isQFCall();
    }
    
    private boolean shallCallObserver()
    {
        return observerHack_ != null && isQFCall();
    }
	...
	private void handleSetData(Event event) {
		...
		// This seems to be called only while the table is being constructed. After the table is contructed
		// (hopefully) setProposals method is called. So, this is not needed?
        // XXX Kivanc starts here...
//        if (speculationHack_ != null)
//            speculationHack_.tableUpdated();
	...
	private ICompletionProposal getSelectedProposal() {
		...
		// XXX Kivanc starts here.
		return doGetSelectedProposal(i);
        // XXX Kivanc ends here.
	}
	
	// XXX Kivanc starts here.
	private ICompletionProposal doGetSelectedProposal(int index)
	{
	    ICompletionProposal result = null;
	    if (shallCallSpeculator())
            // This cast cannot fail!
            result = (ICompletionProposal) speculationHack_.getSelectedProposal(index);
        // This might happen if we haven't ordered the proposals in the UI yet.
        if (result == null)
        {
            /*
             ...
             */
            if (fFilteredProposals == null || index < 0 || index >= fFilteredProposals.length)
                return null;
            // XXX Kivanc changed location of this block: if was after: if (!Helper.okToUse(fProposalTable)) return
            // null;
            result = fFilteredProposals[index];
        }
        return result;
	}
	...
	private void selectProposal(int index, boolean smartToggle) {
		...
		// XXX Kivanc hack starts here.
		// This was called when I change the proposals with keys (i.e., up and down) and if global best proposal is added
		// this results with an index out of bounds exception. That is why we are asking QFS for the selected proposal.
//        ICompletionProposal proposal= fFilteredProposals[index];
		ICompletionProposal proposal = doGetSelectedProposal(index);
		// XXX Kivanc hack ends here.
