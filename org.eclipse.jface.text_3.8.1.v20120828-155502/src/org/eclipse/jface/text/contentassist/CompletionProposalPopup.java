...
class CompletionProposalPopup implements IContentAssistListener {
    ...
    // XXX Kivanc starts here...
    // Note to myself. This class (CompletionProposalPopup is instantiated only once - per
    // contentAssistant)
    ...
    private static ICompletionProposalPopupSpeculationHack getSpeculationHack()
    {
        ...
        if (speculationHack_ == null)
            speculationHack_ = SpeculationHackActionManager.getInstance()
                                                           .getCorrectionProposalPopupHandler();
    ...
    private static ICompletionProposalPopupUsageHack getObserverHack()
    {
        ...
        if (observerHack_ == null)
            observerHack_ = ObserverHackActionManager.getInstance()
                                                     .getCorrectionProposalPopupHandler();
    ...
    private boolean isQFCall()
    {
        if (fContentAssistant == null)
            return false;
        return fContentAssistant.getClass()
                                .getName()
                                .equals("org.eclipse.jface.text.quickassist.QuickAssistAssistant$QuickAssistAssistantImpl");
    }
    ...
    private ICompletionProposal doGetSelectedProposal(int index)
    {
        ...
        {
            /*
             * XXX Kivanc changed location of this block: if was after: if
             * (!Helper.okToUse(fProposalTable)) return null; This blocks position must be changed
             * since if I have calculated the augmented proposal, with the addition of best
             * proposals, the selected inside may be out of the array indices. It we check this
             * before, then it automatically returns null.
             */
            if (fFilteredProposals == null || index < 0 || index >= fFilteredProposals.length)
                return null;
            // XXX Kivanc changed location of this block: if was after: if
            // (!Helper.okToUse(fProposalTable)) return
            // null;
