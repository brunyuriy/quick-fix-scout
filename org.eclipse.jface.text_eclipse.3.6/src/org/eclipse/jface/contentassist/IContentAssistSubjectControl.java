...
public interface IContentAssistSubjectControl
{
     ...
     *             <li>{@link org.eclipse.swt.SWT#ERROR_THREAD_INVALID_ACCESS} - if not called from the thread that
     ...
     * 
     * @return the caret position relative to the start of the text in widget coordinates
     * @throws org.eclipse.swt.SWTException in these cases:
     *             <ul>
     *             <li>{@link org.eclipse.swt.SWT#ERROR_WIDGET_DISPOSED} - if the receiver has been disposed</li>
     *             <li>{@link org.eclipse.swt.SWT#ERROR_THREAD_INVALID_ACCESS} - if not called from the thread that
    ...
    /**
     * Returns the x, y location of the upper left corner of the character bounding box at the specified offset in the
     * text. The point is relative to the upper left corner of the widget client area.
     * 
     * @param offset widget offset relative to the start of the content 0 <= offset <= getCharCount()
     * @return x, y location of the upper left corner of the character bounding box at the specified offset in the text
     * @throws org.eclipse.swt.SWTException in these cases:
     *             <ul>
     *             <li>{@link org.eclipse.swt.SWT#ERROR_WIDGET_DISPOSED} - if the receiver has been disposed</li>
     *             <li>{@link org.eclipse.swt.SWT#ERROR_THREAD_INVALID_ACCESS} - if not called from the thread that
     ...
     * 
     * @return line delimiter used for entering new lines by key down or paste operation
     * @throws org.eclipse.swt.SWTException in these cases:
     *             <ul>
     *             <li>{@link org.eclipse.swt.SWT#ERROR_WIDGET_DISPOSED} - if the receiver has been disposed</li>
     *             <li>{@link org.eclipse.swt.SWT#ERROR_THREAD_INVALID_ACCESS} - if not called from the thread that
     ...
     * 
     * @return start and length of the selection, x is the offset and y the length based on the subject's model (e.g.
     *         document)
     */
    ...
    /**
     * If supported, appends a verify key listener to the viewer's list of verify key listeners. If the listener is
     * already registered with the viewer this call moves the listener to the end of the list.
     * <p>
     * Note: This content assist subject control may not support appending a verify listener, in which case
     * <code>false</code> will be returned. If this content assist subject control only supports
     * <code>addVerifyKeyListener</code> then this method can be used but <code>prependVerifyKeyListener</code> must
    ...
    /**
     * If supported, inserts the verify key listener at the beginning of this content assist subject's list of verify
     * key listeners. If the listener is already registered with the viewer this call moves the listener to the
     * beginning of the list.
     * <p>
     * Note: This content assist subject control may not support prepending a verify listener, in which case
     * <code>false</code> will be returned. However, {@link #appendVerifyKeyListener(VerifyKeyListener)} might work.
     * </p>
     * 
    ...
    /**
     * Removes the verify key listener from this content assist subject control's list of verify key listeners. If the
     * listener is not registered, this call has no effect.
     * 
    ...
    /**
     * Tests whether a verify key listener can be added either using <code>prependVerifyKeyListener</code> or
    ...
    /**
     * Adds the listener to the collection of listeners who will be notified when keys are pressed and released on the
     * system keyboard, by sending it one of the messages defined in the {@link KeyListener} interface.
     * 
    ...
    /**
     * Removes the listener from the collection of listeners who will be notified when keys are pressed and released on
     * the system keyboard.
     * 
     * @param keyListener the listener which should be notified
     ...
     * 
     * @param eventConsumer the content assist subject control's event consumer. <code>null</code> is a valid argument.
     */
     ...
     *         <ul>
     *         <li>{@link org.eclipse.swt.SWT#ERROR_WIDGET_DISPOSED} - if the receiver has been disposed</li> <li>
     *         {@link org.eclipse.swt.SWT#ERROR_THREAD_INVALID_ACCESS} - if not called from the thread that created the
     *         receiver</li>
     *         </ul>
     * @exception IllegalArgumentException if listener is <code>null</code>
     */
