...
class CompletionProposalPopup2 implements IContentAssistListener2
{
    ...
    private final IEditingSupport fModificationEditingSupport = new IEditingSupport()
    {
        public boolean isOriginator(DocumentEvent event, IRegion focus)
        {
            if (fViewer != null)
            {
                ...
                return selection.x <= focus.getOffset() + focus.getLength()
                        && selection.x + selection.y >= focus.getOffset();
            }
            return false;
        }
     ...
     * @since 2.0
     */
    public CompletionProposalPopup2(ContentAssistant2 contentAssistant, ITextViewer viewer,
            AdditionalInfoController2 infoController)
    {
    ...
    public String showProposals(final boolean autoActivated)
    {
        ...
        // if (fComputedProposals == null) fComputedProposals=
        // computeProposals(fContentAssistant.getCompletionPosition());
    ...
    private void createProposalSelector()
    {
        ...
        fContentAssistant.addToLayout(this, fProposalShell, ContentAssistant2.LayoutManager.LAYOUT_PROPOSAL_SELECTOR,
    ...
    private void resizeProposalSelector(boolean adjustWidth)
    {
        ...
        data.heightHint = Math.min(getTableHeightHint(fProposalTable, fProposalTable.getItemCount()),
                getTableHeightHint(fProposalTable, 10));
        fProposalTable.setLayoutData(data);
        ...
        if (key == 0)
        {
            ...
            switch (e.keyCode)
            {
                ...
                case SWT.END:
                    ...
                    break;
                default:
                    if (e.keyCode != SWT.MOD1 && e.keyCode != SWT.MOD2 && e.keyCode != SWT.MOD3
                            && e.keyCode != SWT.MOD4)
                        hide();
                    return true;
            }
    ...
    /**
     * Computes the subset of already computed propsals that are still valid for the given offset.
     * 
     * @param offset the offset
    ...
    private ICompletionProposal [] computeFilteredProposals(int offset, DocumentEvent event)
    {
        ...
        if (offset < fInvocationOffset) { return null; }
