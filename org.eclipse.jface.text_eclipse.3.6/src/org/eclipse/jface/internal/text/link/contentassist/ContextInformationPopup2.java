 ...
 * there was only a single possible context, the context information is shown in a tooltip like popup.
 * <p>
 * If the tooltip is visible and the user wants to see context information of a context embedded into the one for which
...
class ContextInformationPopup2 implements IContentAssistListener2
{
    ...
    private void internalShowContextInfo(IContextInformation information, int offset)
    {
        ...
        if (validator != null)
        {
            ...
            current.fBeginOffset = (information instanceof IContextInformationExtension) ? ((IContextInformationExtension) information)
        ...
        if (initial)
        {
            if (fContentAssistant.addContentAssistListener(this, ContentAssistant2.CONTEXT_INFO_POPUP))
            {
                fContentAssistant.addToLayout(this, fContextInfoPopup,
                        ContentAssistant2.LayoutManager.LAYOUT_CONTEXT_INFO_POPUP, frame.fVisibleOffset);
    ...
    private void createContextSelector()
    {
        ...
        fContentAssistant.addToLayout(this, fContextSelectorShell,
                ContentAssistant2.LayoutManager.LAYOUT_CONTEXT_SELECTOR, fContentAssistant.getSelectionOffset());
    }
    ...
    private void validateContextInformation()
    {
         ...
         * already beed executed. Otherwise, we'd validate the context information based on the pre-key-stroke state.
         */
        fContextInfoPopup.getDisplay().asyncExec(new Runnable()
        {
            ...
            public void run()
            {
                if (Helper2.okToUse(fContextInfoPopup) && fFrame == fContextFrameStack.peek())
                {
                    ...
                    else if (fFrame.fPresenter != null
                            && fFrame.fPresenter.updatePresentation(offset, fTextPresentation))
                    {
                        TextPresentation.applyTextPresentation(fTextPresentation, fContextInfoText);
                        resize();
                    }
