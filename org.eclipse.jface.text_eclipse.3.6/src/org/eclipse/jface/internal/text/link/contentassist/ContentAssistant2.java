...
/**
 * A custom implementation of the <code>IContentAssistant</code> interface. This implementation is used by the linked
 * mode UI. This is internal and subject to change without notice.
 */
public class ContentAssistant2 implements IContentAssistant, IContentAssistantExtension, IWidgetTokenKeeper,
        IWidgetTokenKeeperExtension
{
    ...
    class LayoutManager implements Listener
    {
        ...
        public void handleEvent(Event event)
        {
            ...
            switch (type)
            {
                case LAYOUT_PROPOSAL_SELECTOR:
                    if (fContextType == LAYOUT_CONTEXT_SELECTOR && Helper2.okToUse(fShells[LAYOUT_CONTEXT_SELECTOR]))
                    {
                        // Restore event notification to the tip popup.
                        addContentAssistListener((IContentAssistListener2) fPopups[LAYOUT_CONTEXT_SELECTOR],
                                CONTEXT_SELECTOR);
                    }
                    break;
                case LAYOUT_CONTEXT_SELECTOR:
                    if (Helper2.okToUse(fShells[LAYOUT_PROPOSAL_SELECTOR]))
                    {
                        ...
                        addContentAssistListener((IContentAssistListener2) fPopups[LAYOUT_PROPOSAL_SELECTOR],
                                PROPOSAL_SELECTOR);
                    }
        ...
        protected void layoutProposalSelector(int offset)
        {
            ...
            else if (fContextType != LAYOUT_CONTEXT_SELECTOR || !Helper2.okToUse(fShells[LAYOUT_CONTEXT_SELECTOR]))
            {
            ...
            switch (type)
            {
                case LAYOUT_PROPOSAL_SELECTOR:
                    if (fContextType == LAYOUT_CONTEXT_SELECTOR && Helper2.okToUse(fShells[LAYOUT_CONTEXT_SELECTOR]))
                        ...
                        removeContentAssistListener((IContentAssistListener2) fPopups[LAYOUT_CONTEXT_SELECTOR],
                                CONTEXT_SELECTOR);
                    break;
                case LAYOUT_CONTEXT_SELECTOR:
                    if (Helper2.okToUse(fShells[LAYOUT_PROPOSAL_SELECTOR]))
                        // Disable event notification to the proposal selector.
                        removeContentAssistListener((IContentAssistListener2) fPopups[LAYOUT_PROPOSAL_SELECTOR],
                                PROPOSAL_SELECTOR);
                    break;
                case LAYOUT_CONTEXT_INFO_POPUP:
                    break;
    ...
    /**
     * The popup priority: &gt; info pop-ups, &lt; standard content assist. Default value: <code>10</code>.
     * 
     * @since 3.0
     */
     ...
     * assistant is activated after a 500 ms delay. It uses the default partitioning.
     */
    ...
    ICompletionProposal [] computeCompletionProposals(ITextViewer viewer, int position)
    {
        ...
        else if (fProposalStrings != null)
        {
            ...
            {
                result[i] = new CompletionProposal(fProposalStrings[i], position, fProposalStrings[i].length(),
                        fProposalStrings[i].length());
            }
            return result;
        }
        else
