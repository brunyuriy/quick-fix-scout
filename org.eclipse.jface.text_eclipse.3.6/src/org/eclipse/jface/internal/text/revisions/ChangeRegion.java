...
/**
 * A change region describes a contiguous range of lines that was changed in the same revision of a document. Once it is
 * adjusted to diff information, the originally contiguous range may be split into several ranges or even be empty.
 * 
 * @since 3.2
 */
public final class ChangeRegion
{
    ...
    /**
     * Returns the list of {@link ILineRange}s of this change region for which the revision information is still valid.
     * 
    ...
    public void adjustTo(Hunk hunk)
    {
        for (ListIterator it = fAdjusted.listIterator(); it.hasNext();)
        {
            ...
            if (unchanged > 0)
            {
                ...
                it.add(range);
                it.previous();
                it.next(); // needed so we can remove below
            }
