...
{
    /*
     * Implementation note: Color computation assumes sRGB, which is probably not true, and does not always give good
     * results. CIE based algorithms would be better, see http://www.w3.org/TR/PNG-ColorAppendix.html and
     * http://en.wikipedia.org/wiki/Lab_color_space
     */
    /**
     * Returns the human-perceived brightness of a color as float in [0.0, 1.0]. The used RGB weights come from
    ...
    /**
     * Normalizes a color in its perceived brightness. Yellows are darkened, while blues and reds are lightened.
     * Depending on the hue, the brightness range within the RGB gamut may be different, outside values are clipped.
     * Note that this is an approximation; the returned RGB is not guaranteed to have the requested
    ...
    /**
     * Converts an {@link RGB} to an <a href="http://en.wikipedia.org/wiki/HSL_color_space">HSI</a> triplet.
     * 
    ...
    private static RGB fromHSI(float [] hsi)
    {
        ...
        {
            float temp2 = intensity < 0.5f ? intensity * (1.0f + saturation) : (intensity + saturation)
     ...
     * <p>
     * The returned array has size <code>steps</code>, and the color at index 0 is <code>start</code>, the color at
     * index <code>steps&nbsp;-&nbsp;1</code> is <code>end</code>.
     * 
     ...
     * href="http://en.wikipedia.org/wiki/HSV_color_space">HSB color space</a>. The returned array has size
     * <code>steps</code>. The distance <var>d</var> between two successive colors is in [120&#176;,&nbsp;180&#176;].
     * <p>
     * The color at a given <code>index</code> has the hue returned by {@linkplain #computeHue(int) computeHue(index)};
     * i.e. the computed hues are not equidistant, but adaptively distributed on the color wheel.
     * </p>
     * <p>
     * The first six colors returned correspond to the following {@link SWT} color constants: {@link SWT#COLOR_RED red},
     * {@link SWT#COLOR_GREEN green}, {@link SWT#COLOR_BLUE blue}, {@link SWT#COLOR_YELLOW yellow},
     * {@link SWT#COLOR_CYAN cyan}, {@link SWT#COLOR_MAGENTA magenta}.
     * </p>
     * 
    ...
    /**
     * Returns an indexed hue in [0&#176;,&nbsp;360&#176;), distributing the hues evenly on the hue wheel defined by the
     * <a href="http://en.wikipedia.org/wiki/HSV_color_space">HSB (or HSV) color space</a>. The distance <var>d</var>
     * between two successive colors is in [120&#176;,&nbsp;180&#176;].
     * <p>
     * The first six colors returned correspond to the following {@link SWT} color constants: {@link SWT#COLOR_RED red},
     * {@link SWT#COLOR_GREEN green}, {@link SWT#COLOR_BLUE blue}, {@link SWT#COLOR_YELLOW yellow},
     * {@link SWT#COLOR_CYAN cyan}, {@link SWT#COLOR_MAGENTA magenta}.
    ...
    public static float computeHue(final int index)
    {
        ...
        /*
         * Base 3 gives a nice partitioning for RGB colors with red, green, blue being the colors 0,1,2, and yellow,
         * cyan, magenta colors 3,4,5.
         */
        final int base = 3;
