/*******************************************************************************
 * Copyright (c) 2006, 2009 IBM Corporation and others. All rights reserved. This program and the accompanying materials
...
public final class RevisionPainter
{
    ...
    /**
     * The color tool manages revision colors and computes shaded colors based on the relative age and author of a
     * revision.
     */
    private final class ColorTool
    {
        /**
         * The average perceived intensity of a base color. 0 means black, 1 means white. A base revision color
         * perceived as light such as yellow will be darkened, while colors perceived as dark such as blue will be
         * lightened up.
         */
        private static final float AVERAGE_INTENSITY = 0.5f;
        /**
         * The maximum shading in [0, 1] - this is the shade that the most recent revision will receive.
         */
    ...
    private static final class HoverInformationControlCreator extends AbstractReusableInformationControlCreator
    {
        ...
        /*
         * @see
         * org.eclipse.jface.internal.text.revisions.AbstractReusableInformationControlCreator#doCreateInformationControl
         * (org.eclipse.swt.widgets.Shell)
         */
        protected IInformationControl doCreateInformationControl(Shell parent)
        {
            if (BrowserInformationControl.isAvailable(parent)) { return new BrowserInformationControl(parent,
                    JFaceResources.DIALOG_FONT, fIsFocusable)
            {
                /**
                 * {@inheritDoc}
                 * 
                ...
                /**
                 * Adds a HTML header and CSS info if <code>html</code> is only an HTML fragment (has no &lt;html&gt;
                 * section).
                 * 
        ...
        /*
         * @see org.eclipse.jface.text.AbstractReusableInformationControlCreator#canReplace(org.eclipse.jface.text.
     ...
     */
    private final class RevisionHover implements IAnnotationHover, IAnnotationHoverExtension,
            IAnnotationHoverExtension2, IInformationProviderExtension2
    {
        ...
        /*
         * @see
         * org.eclipse.jface.text.source.IAnnotationHoverExtension#getHoverInfo(org.eclipse.jface.text.source.ISourceViewer
        ...
        /*
         * @see org.eclipse.jface.text.source.IAnnotationHoverExtension#getHoverLineRange(org.eclipse.jface.text.source.
         * ISourceViewer, int)
         */
    ...
    /**
     * The currently selected revision, <code>null</code> if none. The difference between {@link #fFocusRevision} and
     * {@link #fSelectedRevision} may not be obvious: the focus revision is the one focused by the mouse (by hovering
     * over a block of the revision), while the selected revision is sticky, i.e. is not removed when the mouse leaves
     * the ruler.
     * 
     * @since 3.3
     */
     ...
     * 
     * @param column the column that will delegate{@link #paint(GC, ILineRange) painting} to the newly created painter.
     ...
     * 
     * @param background the background color, <code>null</code> for the platform's list background
     */
    ...
    /**
     * Delegates the painting of the quick diff colors to this painter. The painter will draw the color boxes onto the
     * passed {@link GC} for all model (document) lines in <code>visibleModelLines</code>.
     * 
     * @param gc the {@link GC} to draw onto
     * @param visibleLines the lines (in document offsets) that are currently (perhaps only partially) visible
     */
    ...
    /**
     * Ensures that the column is fully instantiated, i.e. has a control, and that the viewer is visible.
     */
    ...
    /**
     * Returns the revision range that contains the given line, or <code>null</code> if there is none.
     * 
    ...
    /**
     * Gets all change ranges of the revisions in the revision model and adapts them to the current quick diff
     * information. The list is cached.
     * 
     ...
     * it is the range's exclusive end line.
     * 
     * @param range the range to check whether it contains <code>line</code>
     * @param line the line the line
     * @return <code>true</code> if <code>range</code> contains <code>line</code>, <code>false</code> if not
     */
    ...
    /**
     * Computes and returns the bounds of the rectangle corresponding to a widget line range. The rectangle is in pixel
     * coordinates relative to the text widget's {@link StyledText#getClientArea() client area} and has the width of the
     * ruler.
     * 
    ...
    private void installWheelHandler()
    {
        ...
        {
            // FIXME: does not work on Windows, because Canvas cannot get focus and therefore does not send out mouse
            // wheel events:
    ...
    private void handleMouseWheel(Event event)
    {
        ...
        int newTopPixel = fWidget.getTopPixel()
                + JFaceTextUtil.computeLineHeight(fWidget, widgetCurrentFocusLine, widgetNextFocusLine,
                        widgetNextFocusLine - widgetCurrentFocusLine);
    ...
    /**
     * Translates a y coordinate in the pixel coordinates of the column's control to a document line number.
     * 
    ...
    {
        return fViewer instanceof ISourceViewer
                && fHover.getHoverLineRange((ISourceViewer) fViewer, activeLine) != null;
    }
