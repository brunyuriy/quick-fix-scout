 ...
 * <p>
 * In order to provide backward compatibility for clients of <code>IHyperlinkDetector</code>, extension interfaces are
 * used to provide a means of evolution. The following extension interfaces exist:
 * <ul>
 * <li>{@link IHyperlinkDetectorExtension} since version 3.3, adds the ability to dispose a hyperlink detector</li>
 * <li>{@link IHyperlinkDetectorExtension2} since version 3.3, adds the ability to specify the state mask of the
 * modifier keys that need to be pressed for this hyperlink detector</li>
 * </ul>
 * </p>
 * <p>
...
public interface IHyperlinkDetector
{
     ...
     * @param canShowMultipleHyperlinks tells whether the caller is able to show multiple links to the user. If
     *            <code>true</code> {@link IHyperlink#open()} should directly open the link and not show any additional
     *            UI to select from a list. If <code>false</code> this method should only return one hyperlink which
     *            upon {@link IHyperlink#open()} may allow to select from a list.
