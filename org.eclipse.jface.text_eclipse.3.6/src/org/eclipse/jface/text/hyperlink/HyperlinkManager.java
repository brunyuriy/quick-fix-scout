 ...
 * Steffen Pingel <steffen.pingel@tasktop.com> (Tasktop Technologies Inc.) - [navigation] hyperlink decoration is not
 * erased when mouse is moved out of Text widget - https://bugs.eclipse.org/bugs/show_bug.cgi?id=100278
 *******************************************************************************/
 ...
 * @since 3.1
 */
public class HyperlinkManager implements ITextListener, Listener, KeyListener, MouseListener, MouseMoveListener,
        FocusListener, MouseTrackListener
{
    /**
     ...
     * <p>
     * This strategy is only allowed if {@link IHyperlinkPresenter#canShowMultipleHyperlinks()} returns
     * <code>true</code>.
     * </p>
     */
    public static final DETECTION_STRATEGY ALL = new DETECTION_STRATEGY("all"); //$NON-NLS-1$
    /**
     * All detected hyperlinks from all detectors are collected and all those with the longest region are passed to the
     * hyperlink presenter.
     * <p>
     * This strategy is only allowed if {@link IHyperlinkPresenter#canShowMultipleHyperlinks()} returns
     * <code>true</code>.
    ...
    /**
     * All detected hyperlinks from all detectors are collected and form all those with the longest region only the
     * first one is passed to the hyperlink presenter.
     */
     ...
     * 
     * @param detectionStrategy the detection strategy one of {{@link #ALL}, {@link #FIRST}, {@link #LONGEST_REGION_ALL}
            ...
            IHyperlinkDetector [] hyperlinkDetectors, int eventStateMask)
    {
        ...
        Assert.isLegal(fHyperlinkPresenter.canShowMultipleHyperlinks() || fDetectionStrategy == FIRST
                || fDetectionStrategy == LONGEST_REGION_FIRST);
