 ...
 * <p>
 * The two main methods are <code>findAndSelect</code> and <code>replaceSelection</code>. The target does not provide
 * any way to modify the content other than replacing the selection.
 * <p>
 * In order to provide backward compatibility for clients of <code>IFindReplaceTarget</code>, extension interfaces are
 * used as a means of evolution. The following extension interfaces exist:
 * <ul>
 * <li>{@link org.eclipse.jface.text.IFindReplaceTargetExtension} since version 2.0 introducing the notion of
 * find/replace session and of a find/replace scope. In additions, in allows clients to replace all occurrences of a
 * given find query.</li>
 * <li>{@link org.eclipse.jface.text.IFindReplaceTargetExtension3} since version 3.0 allowing clients to specify search
 * queries as regular expressions.</li>
 * </ul>
 * <p>
 * Clients of a <code>IFindReplaceTarget</code> that also implements the <code>IFindReplaceTargetExtension</code> have
 * to indicate the start of a find/replace session before using the target and to indicate the end of the session when
 * the target is no longer used.
 * 
...
public interface IFindReplaceTarget
{
     ...
     * @param wholeWord if <code>true</code> only occurrences are reported in which the findString stands as a word by
     *            itself
     * @return the position of the specified string, or -1 if the string has not been found
     */
    int findAndSelect(int widgetOffset, String findString, boolean searchForward, boolean caseSensitive,
            boolean wholeWord);
    ...
    /**
     * Replaces the currently selected range of characters with the given text. This target must be editable. Otherwise
     * nothing happens.
     * <p>
