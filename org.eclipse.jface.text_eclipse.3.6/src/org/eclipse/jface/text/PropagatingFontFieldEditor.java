...
/**
 * This font field editor implements chaining between a source preference store and a target preference store. Any time
 * the source preference store changes, the change is propagated to the target store. Propagation means that the actual
 * value stored in the source store is set as default value in the target store. If the target store does not contain a
 * value other than the default value, the new default value is immediately effective.
 * 
 * @see FontFieldEditor
 * @since 2.0
...
public class PropagatingFontFieldEditor extends FontFieldEditor
{
    ...
    /**
     * Checks whether this editor presents the default value "inherited" from the workbench rather than its own font.
     */
    ...
    /**
     * Propagates the font set in the source store to the target store using the given keys.
     * 
    ...
    /**
     * Starts the propagation of the font preference stored in the source preference store under the source key to the
     * target preference store using the target preference key.
     * 
     ...
     */
    public static void startPropagate(final IPreferenceStore source, final String sourceKey,
            final IPreferenceStore target, final String targetKey)
    {
