 ...
 * issued by the text widget of a text viewer. It overrides <code>computeInformation</code>, so that the computation is
 * performed in a dedicated background thread. This implies that the used <code>ITextHover</code> objects must be
 * capable of operating in a non-UI thread.
 * 
 * @since 2.0
 */
        ...
        IWidgetTokenKeeperExtension
{
    ...
    /**
     * Creates a new text viewer hover manager specific for the given text viewer. The manager uses the given
            ...
            public void run()
            {
                ...
                try
                {
                    if (fThread != null)
                    {
                        ...
                        {
                            /*
                             * This code runs in a separate thread which can lead to text offsets being out of bounds
                             * when computing the hover info (see bug 32848).
                             */
                            information = null;
                        }
                ...
                catch (RuntimeException ex)
                {
                    ...
                    log.log(new Status(IStatus.ERROR, PLUGIN_ID, IStatus.OK,
                            "Unexpected runtime error while computing a text hover", ex)); //$NON-NLS-1$
                }
                finally
                {
                    synchronized (fMutex)
    ...
    /**
     * As computation is done in the background, this method is also called in the background thread. Delegates the
     * control flow back into the UI thread, in order to allow displaying the information in the information control.
     */
    ...
    /**
     * Computes the document offset underlying the given text widget coordinates. This method uses a linear search as it
     * cannot make any assumption about how the document is actually presented in the widget. (Covers cases such as
     * bidirectional text.)
     * 
    ...
    /**
     * Returns the currently shown text hover or <code>null</code> if no text hover is shown.
     * 
