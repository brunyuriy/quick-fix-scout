 ...
 * <p>
 * It registers with the connected text viewer as text input listener and document listener and logs all changes. It
 * also monitors mouse and keyboard activities in order to partition the stream of text changes into undo-able edit
 * commands.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * The usage of {@link org.eclipse.core.runtime.IAdaptable} in the JFace layer has been approved by Platform UI, see
 * https://bugs.eclipse.org/bugs/show_bug.cgi?id=87669#c9
 * </p>
 * <p>
...
public class DefaultUndoManager implements IUndoManager, IUndoManagerExtension
{
    ...
    class TextCommand extends AbstractOperation
    {
        ...
        protected void undoTextChange()
        {
            try
            {
                ...
                if (document instanceof IDocumentExtension4)
                    ((IDocumentExtension4) document).replace(fStart, fText.length(), fPreservedText,
        ...
        public boolean canUndo()
        {
            if (isConnected() && isValid())
            {
                ...
                if (doc instanceof IDocumentExtension4)
                {
                    ...
                    /*
                     * Special case to check if the answer is false. If the last document change was empty, then the
                     * document's modification stamp was incremented but nothing was committed. The operation being
                     * queried has an older stamp. In this case only, the comparison is different. A sequence of
                     * document changes that include an empty change is handled correctly when a valid commit follows
                     * the empty change, but when #canUndo() is queried just after an empty change, we must special case
                     * the check. The check is very specific to prevent false positives. see
                     * https://bugs.eclipse.org/bugs/show_bug.cgi?id=98245
                     */
                    if (!canUndo && this == fHistory.getUndoOperation(fUndoContext) && // this is the latest operation
                    ...
                    /*
                     * When the composite is the current command, it may hold the timestamp of a no-op change. We check
                     * this here rather than in an override of canUndo() in CompoundTextCommand simply to keep all the
                     * special case checks in one place.
                     */
                    if (!canUndo && this == fHistory.getUndoOperation(fUndoContext) && // this is the latest operation
                            this instanceof CompoundTextCommand && this == fCurrent && // this is the current operation
                            this.fStart == -1 && // the current operation text is not valid
                            fCurrent.fRedoModificationStamp != IDocumentExtension4.UNKNOWN_MODIFICATION_STAMP)
        ...
        /*
         * @see
         * org.eclipse.core.commands.operations.IUndoableOperation#execute(org.eclipse.core.runtime.IProgressMonitor,
        ...
        /*
         * Undo the change described by this command. Also selects and reveals the change.
         */
        /**
         * Undo the change described by this command. Also selects and reveals the change.
         * 
        ...
        /**
         * Re-applies the change described by this command that previously been rolled back. Also selects and reveals
         * the change.
         * 
         * @param monitor the progress monitor to use if necessary
         * @param uiInfo an adaptable that can provide UI info if needed
        ...
        /**
         * Attempt a commit of this command and answer true if a new fCurrent was created as a result of the commit.
         * 
         * @return true if the command was committed and created a new fCurrent, false if not.
         * @since 3.1
         */
    ...
    /**
     * The operation history listener used for managing undo and redo before and after the individual commands are
     * performed.
     * 
     * @since 3.1
     */
    ...
    /**
     * The command last added to the operation history. This must be tracked internally instead of asking the history,
     * since outside parties may be placing items on our undo/redo history.
     */
    ...
    /**
     * Adds the given command to the operation history if it is not part of a compound change.
     * 
     * @param command the command to be added
     * @since 3.1
     */
    ...
    /**
     * Checks whether the given text starts with a line delimiter and subsequently contains a white space only.
     * 
    ...
    private void processChange(int modelStart, int modelEnd, String insertedText, String replacedText,
            long beforeChangeModificationStamp, long afterChangeModificationStamp)
    {
