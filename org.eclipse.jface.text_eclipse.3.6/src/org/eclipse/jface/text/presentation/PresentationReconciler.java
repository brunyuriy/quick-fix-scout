...
/**
 * Standard implementation of <code>IPresentationReconciler</code>. This implementation assumes that the tasks performed
 * by its presentation damagers and repairers are lightweight and of low cost. This presentation reconciler runs in the
 * UI thread and always repairs the complete damage caused by a document change rather than just the portion overlapping
 * with the viewer's viewport.
 * <p>
...
public class PresentationReconciler implements IPresentationReconciler, IPresentationReconcilerExtension
{
    ...
    /**
     * Creates a new presentation reconciler. There are no damagers or repairers registered with this reconciler by
     * default. The default partitioning <code>IDocumentExtension3.DEFAULT_PARTITIONING</code> is used.
     */
    ...
    /**
     * Registers the given presentation damager for a particular content type. If there is already a damager registered
     * for this type, the old damager is removed first.
     * 
    ...
    /**
     * Registers the given presentation repairer for a particular content type. If there is already a repairer
     * registered for this type, the old repairer is removed first.
     * 
    ...
    /**
     * Constructs a "repair description" for the given damage and returns this description as a text presentation. For
     * this, it queries the partitioning of the damage region and asks the appropriate presentation repairer for each
     * partition to construct the "repair description" for this partition.
     * 
     * @param damage the damage to be repaired
     * @param document the document whose presentation must be repaired
     * @return the presentation repair description as text presentation or <code>null</code> if the partitioning could
     *         not be computed
     */
    ...
    /**
     * Checks for the first and the last affected partition affected by a document event and calls their damagers.
     * Invalidates everything from the start of the damage for the first partition until the end of the damage for the
     * last partition.
     * 
     ...
     * @return the damaged caused by the change or <code>null</code> if computing the partitioning failed
     * @since 3.0
     */
    ...
    /**
     * Returns the end offset of the damage. If a partition has been split by the given document event also the second
     * half of the original partition must be considered. This is achieved by using the remembered partition range.
     * 
