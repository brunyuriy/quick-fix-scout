 ...
 * <p>
 * Replaces the original <code>requestWidgetToken</code> functionality with a new priority based approach.
 * 
 * @since 3.0
 */
public interface IWidgetTokenOwnerExtension
{
     ...
     * <p>
     * <code>priority</code> is forwarded to any existing token keeper to give it an estimate on whether the request has
     * higher priority than the current keeper's. There is, however, no guarantee that another keeper will release the
     * token even if it has a high priority.
     * </p>
     * 
