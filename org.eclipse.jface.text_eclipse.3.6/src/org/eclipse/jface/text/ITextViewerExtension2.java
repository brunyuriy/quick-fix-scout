 ...
 * </ul>
 * It extends the means for text presentation invalidation by allowing a specific region of the presentation to get
 * invalidated. It replaces {@link org.eclipse.jface.text.ITextViewer#setTextHover(ITextHover, String)} with a new
 * method that allows to specify state masks for a better control of the hover behavior.
 * <p>
 * An {@link org.eclipse.jface.text.IAutoEditStrategy} is a generalization of the original
 * {@link org.eclipse.jface.text.IAutoIndentStrategy}. Auto edit strategies can be arranged in a list that is executed
 * like a pipeline when the viewer content is changed.
 * <p>
 * A {@link org.eclipse.jface.text.IPainter}is creating and managing visual decorations on the viewer's text widget.
 * Viewer's can have an open number of painters. Painters are informed about changes to the viewer content and state and
 * can take the necessary action in responds to the notification.
 * 
 * @since 2.1
 */
public interface ITextViewerExtension2
{
    ...
    /**
     * Sets this viewer's text hover for the given content type and the given state mask. If the given text hover is
     * <code>null</code>, any hover installed for the given content type and state mask is removed.
     * 
     ...
     * @param stateMask the SWT event state mask; <code>DEFAULT_HOVER_STATE_MASK</code> indicates that the hover is
     *            installed as the default hover.
     */
     ...
     * <p>
     * Note: To remove a hover for a given content type and state mask use
     * {@link #setTextHover(ITextHover, String, int)} with <code>null</code> as parameter for the text hover.
     * </p>
     * 
    ...
    /**
     * Prepends the given auto edit strategy to the existing list of strategies for the specified content type. The
     * strategies are called in the order in which they appear in the list of strategies.
     * 
    ...
    /**
     * Removes the first occurrence of the given auto edit strategy in the list of strategies registered under the
    ...
    /**
     * Removes the given painter from this viewer. If the painter has not been added to this viewer, this call is
     * without effect.
     * 
