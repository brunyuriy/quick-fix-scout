 ...
 * Tom Eicher (Avaloq Evolution AG) - block selection mode Anton Leherbauer <anton.leherbauer@windriver.com> -
 * [projection] Eclipse is too eager to unfold code - http://bugs.eclipse.org/178203
 *******************************************************************************/
...
/**
 * A projection source viewer is a source viewer which supports multiple visible regions which can dynamically be
 * changed.
 * <p>
 * A projection source viewer uses a <code>ProjectionDocumentManager</code> for the management of the visible document.
 * </p>
 * <p>
...
public class ProjectionViewer extends SourceViewer implements ITextViewerExtension5
{
    ...
    {
        /*
         * @see org.eclipse.jface.text.source.IAnnotationModelListener#modelChanged(org.eclipse.jface.text.source.
         * IAnnotationModel)
         */
    ...
    /**
     * A command representing a change of the projection document. This can be either adding a master document range,
     * removing a master document change, or invalidating the viewer text presentation.
     */
    ...
    /**
     * The amount of lines deleted by the last document event issued by the visible document event.
     * 
     * @since 3.1
     */
     ...
     */
    public ProjectionViewer(Composite parent, IVerticalRuler ruler, IOverviewRuler overviewRuler,
    ...
    /*
     * @see
     * org.eclipse.jface.text.source.SourceViewer#createVisualAnnotationModel(org.eclipse.jface.text.source.IAnnotationModel
     * )
     */
    ...
    /**
     * Adds a projection annotation listener to this viewer. The listener may not be <code>null</code>. If the listener
     * is already registered, this method does not have any effect.
     * 
    ...
    /**
     * Removes the given listener from this viewer. The listener may not be <code>null</code>. If the listener is not
     * registered with this viewer, this method is without effect.
     * 
     ...
     * 
     * @return <code>true</code> if this viewer is in projection mode, <code>false</code> otherwise
     */
            ...
            for (Iterator e = fProjectionAnnotationModel.getAnnotationIterator(); e.hasNext();)
            {
                ...
                if (annotation.isCollapsed())
                {
                    ...
                    if (position != null && touches(selection, position))
                        if (found == null || position.includes(found.offset)
                                && position.includes(found.offset + found.length))
                        {
                            found = position;
    ...
    {
        return position.overlapsWith(selection.x, selection.y) || selection.y == 0
                && position.offset + position.length == selection.x + selection.y;
    }
        ...
        if (isProjectionMode())
        {
            ...
            for (Iterator e = fProjectionAnnotationModel.getAnnotationIterator(); e.hasNext();)
            {
                ...
                if (!annotation.isCollapsed())
                {
                    ...
                    // take the first most fine grained match
                    if (position != null && touches(selection, position))
                        if (found == null || found.includes(position.offset)
                                && found.includes(position.offset + position.length))
                        {
                            found = position;
                            bestMatch = annotation;
                        }
    ...
    /**
     * Adds the given master range to the given projection document. While the modification is processed, the viewer no
     * longer handles projection changes, as it is causing them.
     * 
    ...
    /**
     * Removes the given master range from the given projection document. While the modification is processed, the
     * viewer no longer handles projection changes, as it is causing them.
     * 
     * @param projection the projection document
     * @param offset the offset in the master document
     * @param length the length in the master document
    ...
    /**
     * Returns the first line offset &lt;= <code>offset</code>. If <code>testLastLine</code> is <code>true</code> and
     * the offset is on last line then <code>offset</code> is returned.
     * 
    ...
    /**
     * Makes the given range visible again while not changing the folding state of any contained ranges. If requested, a
     * redraw request is issued.
     * 
    ...
    /**
     * Processes the request for catch up with the annotation model in the UI thread. If the current thread is not the
     * UI thread or there are pending catch up requests, a new request is posted.
     * 
     ...
     * 
     * @return <code>true</code> if the visible document's master is identical to this viewer's document
     * @since 3.1
     */
    ...
    /**
     * Adapts the slave visual document of this viewer to the changes described in the annotation model event. When the
     * event is <code>null</code>, this is identical to a world change event.
     * 
     * @param event the annotation model event or <code>null</code>
     * @exception BadLocationException in case the annotation model event is no longer in synchronization with the
     *                document
     */
    ...
    private void executeProjectionCommands(ProjectionCommandQueue commandQueue, boolean fireRedraw)
            throws BadLocationException
    {
        ProjectionCommand command;
    ...
    /*
     * We pass the removed annotation into this method for performance reasons only. Otherwise, they could be fetch from
     * the event.
     */
    private void processDeletions(AnnotationModelEvent event, Annotation [] removedAnnotations, boolean fireRedraw)
            throws BadLocationException
    {
    ...
    /**
     * Computes the region that must be collapsed when the given position is the position of an expanded projection
     * annotation.
     * 
    ...
    /**
     * Computes the regions that must be collapsed when the given position is the position of an expanded projection
     * annotation.
     * 
     * @param position the position
     * @return the ranges that must be collapsed, or <code>null</code> if there are none
     * @since 3.1
     */
    ...
    /**
     * Computes the collapsed region anchor for the given position. Assuming that the position is the position of an
     * expanded projection annotation, the anchor is the region that is still visible after the projection annotation
     * has been collapsed.
     * 
     * @param position the position
    ...
    private void processChanges(Annotation [] annotations, boolean fireRedraw, List coverage)
            throws BadLocationException
    {
    ...
    private boolean covers(List coverage, Position position)
    {
        ...
        while (e.hasNext())
        {
            ...
            if (p.getOffset() <= position.getOffset()
                    && position.getOffset() + position.getLength() <= p.getOffset() + p.getLength())
                return true;
        }
        return false;
    }
    ...
    /**
     * Forces this viewer to throw away any old state and to initialize its content from its projection annotation
     * model.
     * 
    ...
    public void setRangeIndication(int offset, int length, boolean moveCursor)
    {
        ...
        if (moveCursor
                && fProjectionAnnotationModel != null
                && (rangeIndication == null || offset != rangeIndication.getOffset() || length != rangeIndication
                        .getLength()))
        {
            ...
            while (iterator.hasNext())
            {
                ...
                if (annotation.isCollapsed()
                        && willAutoExpand(fProjectionAnnotationModel.getPosition(annotation), offset, length))
    ...
    /*
     * @see
     * org.eclipse.jface.text.TextViewer#handleVisibleDocumentAboutToBeChanged(org.eclipse.jface.text.DocumentEvent)
     */
    ...
    protected void copyMarkedRegion(boolean delete)
    {
        ...
        if (markedRegion != null)
            copyToClipboard(new TextSelection(getDocument(), markedRegion.getOffset(), markedRegion.getLength()),
    ...
    private void copyToClipboard(ITextSelection selection, boolean delete, StyledText textWidget)
    {
        ...
        {
            /*
             * XXX: Reduce pain of https://bugs.eclipse.org/bugs/show_bug.cgi?id=64498 by letting the widget handle the
             * copy operation in this special case.
             */
            textWidget.copy();
        }
            ...
            try
            {
                ...
                catch (SWTError e)
                {
                    if (e.code != DND.ERROR_CANNOT_SET_CLIPBOARD)
                        throw e;
                    /*
                     * TODO see https://bugs.eclipse.org/bugs/show_bug.cgi?id=59459 we should either log and/or inform
                     * the user silently fail for now.
                     */
                    return;
                }
            }
    ...
    protected Point widgetSelection2ModelSelection(Point widgetSelection)
    {
        ...
        /*
         * There is one requirement that governs preservation of logical positions: 1) a selection with widget_length ==
         * 0 should never expand to have model_length > 0. There are a number of ambiguities to resolve with projection
         * regions. A projected region P has a widget-length of zero. Its widget offset may interact with the selection
         * S in various ways: A) P.widget_offset lies at the caret, S.widget_length is zero. Requirement 1 applies. S is
         * *behind* P (done so by widgetRange2ModelRange). B) P.widget_offset lies inside the widget selection. This
         * case is easy: P is included in S, which is automatically done so by widgetRange2ModelRange. C)
         * P.widget_offset lies at S.widget_end: This is arguable - our policy is to include P if it belongs to a
         * projection annotation that overlaps with the widget selection. D) P.widget_offset lies at S.widget_offset:
         * Arguable - our policy is to include P if it belongs to a projection annotation that overlaps with the widget
         * selection
         */
    ...
    /**
     * Returns the positions of all annotations that intersect with <code>modelSelection</code> and that are at least
     * partly visible.
     * 
     * @param modelSelection a model range
     * @return the positions of all annotations that intersect with <code>modelSelection</code>
     * @since 3.1
     */
    private Position [] computeOverlappingAnnotationPositions(IRegion modelSelection)
    {
        ...
        for (Iterator e = fProjectionAnnotationModel.getAnnotationIterator(); e.hasNext();)
        {
            ProjectionAnnotation annotation = (ProjectionAnnotation) e.next();
            Position position = fProjectionAnnotationModel.getPosition(annotation);
            if (position != null && position.overlapsWith(modelSelection.getOffset(), modelSelection.getLength())
            ...
            boolean wholeWord, boolean regExSearch)
    {
        ...
        try
        {
            IRegion matchRegion = getFindReplaceDocumentAdapter().find(startPosition, findString, forwardSearch,
    ...
    /*
     * @see org.eclipse.jface.text.TextViewer#findAndSelectInRange(int, java.lang.String, boolean, boolean, boolean,
     * int, int, boolean)
     */
    ...
    {
        if (!isProjectionMode())
            return super.findAndSelectInRange(startPosition, findString, forwardSearch, caseSensitive, wholeWord,
                    rangeOffset, rangeLength, regExSearch);
        StyledText textWidget = getTextWidget();
        if (textWidget == null)
            return -1;
        try
        {
            ...
            IRegion matchRegion = getFindReplaceDocumentAdapter().find(modelOffset, findString, forwardSearch,
                    caseSensitive, wholeWord, regExSearch);
            if (matchRegion != null)
            {
