...
class ProjectionSummary
{
    ...
    /**
     * Adds the given annotation type. For now on, annotations of that type are also reflected in their enclosing
     * collapsed regions.
     * 
    ...
    /**
     * Removes the given annotation. Annotation of that type are no longer reflected in their enclosing collapsed
     * region.
     * 
    ...
    private boolean includes(IRegion [] regions, Position position)
    {
        for (int i = 0; i < regions.length; i++)
        {
            ...
            if (position != null && !position.isDeleted() && region.getOffset() <= position.getOffset()
