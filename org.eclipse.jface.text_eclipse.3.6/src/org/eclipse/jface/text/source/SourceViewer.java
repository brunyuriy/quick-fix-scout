 ...
 * Tom Eicher (Avaloq Evolution AG) - block selection mode Tom Hofmann (Perspectix AG) - bug 297572
 *******************************************************************************/
...
/**
 * SWT based implementation of {@link org.eclipse.jface.text.source.ISourceViewer} and its extension interfaces. The
 * same rules apply as for {@link org.eclipse.jface.text.TextViewer}. A source viewer uses an
 * <code>IVerticalRuler</code> as its annotation presentation area. The vertical ruler is a small strip shown left of
 * the viewer's text widget. A source viewer uses an <code>IOverviewRuler</code> as its presentation area for the
 * annotation overview. The overview ruler is a small strip shown right of the viewer's text widget.
 * <p>
 * Clients are supposed to instantiate a source viewer and subsequently to communicate with it exclusively using the
 * <code>ISourceViewer</code> and its extension interfaces.
 * </p>
 * <p>
...
public class SourceViewer extends TextViewer implements ISourceViewer, ISourceViewerExtension, ISourceViewerExtension2,
        ISourceViewerExtension3, ISourceViewerExtension4
{
    /**
    ...
    protected class RulerLayout extends Layout
    {
        ...
        protected void layout(Composite composite, boolean flushCache)
        {
            ...
            if (fVerticalRuler != null && fIsVerticalRulerVisible)
            {
                ...
                verticalRulerControl.setBounds(clArea.x, clArea.y + topTrim, verticalRulerWidth, clArea.height
                        - scrollbarHeight - topTrim);
            ...
            if (overviewRulerWidth != -1)
            {
                ...
                fOverviewRuler.getControl().setBounds(overviewRulerX, clArea.y + arrowHeights[0], overviewRulerWidth,
                        clArea.height - arrowHeights[0] - arrowHeights[1] - scrollbarHeight);
                Control headerControl = fOverviewRuler.getHeaderControl();
                boolean noArrows = arrowHeights[0] == 0 && arrowHeights[1] == 0;
                if (noArrows || arrowHeights[0] < arrowHeights[1] && arrowHeights[0] < scrollbarHeight
                        && arrowHeights[1] > scrollbarHeight)
                {
                    // // not enough space for header at top => move to bottom
                    int headerHeight = noArrows ? scrollbarHeight : arrowHeights[1];
                    headerControl.setBounds(overviewRulerX, clArea.y + clArea.height - arrowHeights[1]
                            - scrollbarHeight, overviewRulerWidth, headerHeight);
                }
                else
                {
    ...
    /**
     * The size of the gap between the vertical ruler and the text widget (value <code>2</code>).
     * <p>
    ...
    /**
     * The size of the gap between the vertical ruler and the text widget (value <code>1</code>).
     * 
     * @since 3.2
     */
    ...
    /**
     * Constructs a new source viewer. The vertical ruler is initially visible. The viewer has not yet been initialized
     * with a source viewer configuration.
     * 
    ...
    /**
     * Constructs a new source viewer. The vertical ruler is initially visible. The overview ruler visibility is
     * controlled by the value of <code>showAnnotationsOverview</code>. The viewer has not yet been initialized with a
     * source viewer configuration.
     * 
     * @param parent the parent of the viewer's control
     * @param verticalRuler the vertical ruler used by this source viewer
     * @param overviewRuler the overview ruler
     * @param showAnnotationsOverview <code>true</code> if the overview ruler should be visible, <code>false</code>
     *            otherwise
     * @param styles the SWT style bits for the viewer's control,
     *            <em>if <code>SWT.WRAP</code> is set then a custom document adapter needs to be provided, see {@link #createDocumentAdapter()}
     * @since 2.1
    ...
    /**
     * Sets the overview ruler's annotation hover of this source viewer. The annotation hover provides the information
     * to be displayed in a hover popup window if requested over the overview rulers area. The annotation hover is
    ...
    {
        if (fVerticalRuler != null && (fAnnotationHover != null || !isVerticalRulerOnlyShowingAnnotations())
                && fVerticalRulerHoveringController == null && fHoverControlCreator != null)
        {
            fVerticalRulerHoveringController = new AnnotationBarHoverManager(fVerticalRuler, this, fAnnotationHover,
                    fHoverControlCreator);
            fVerticalRulerHoveringController.install(fVerticalRuler.getControl());
            fVerticalRulerHoveringController.getInternalAccessor().setInformationControlReplacer(
                    new StickyHoverManager(this));
        }
    }
    ...
    {
        if (fOverviewRuler != null && fOverviewRulerAnnotationHover != null && fOverviewRulerHoveringController == null
                && fHoverControlCreator != null)
        {
            fOverviewRulerHoveringController = new OverviewRulerHoverManager(fOverviewRuler, this,
                    fOverviewRulerAnnotationHover, fHoverControlCreator);
            fOverviewRulerHoveringController.install(fOverviewRuler.getControl());
            fOverviewRulerHoveringController.getInternalAccessor().setInformationControlReplacer(
                    new StickyHoverManager(this));
        }
    }
    ...
    /*
     * @see
     * org.eclipse.jface.text.TextViewer#setHoverEnrichMode(org.eclipse.jface.text.ITextViewerExtension8.EnrichMode)
     * @since 3.4
     */
    ...
    public boolean canDoOperation(int operation)
    {
        ...
        if (operation == FORMAT) { return fContentFormatter != null && isEditable(); }
    ...
    protected Point rememberSelection()
    {
        ...
        try
        {
            ...
            if (selection instanceof IBlockTextSelection)
                position = new ColumnPosition(selection.getOffset(), selection.getLength(),
    ...
    protected void restoreSelection()
    {
        if (!fSelections.isEmpty())
        {
            ...
            try
            {
                ...
                if (currentSelection == null || currentSelection.x != position.getOffset()
                        || currentSelection.y != position.getLength())
                {
                    if (position instanceof ColumnPosition && getTextWidget().getBlockSelection())
                    {
                        setSelection(new BlockTextSelection(document, document.getLineOfOffset(position.getOffset()),
                                ((ColumnPosition) position).fStartColumn, document.getLineOfOffset(position.getOffset()
                                        + position.getLength()), ((ColumnPosition) position).fEndColumn,
        ...
        switch (operation)
        {
            ...
            case FORMAT:
            {
                ...
                if (document instanceof IDocumentExtension4)
                {
                    ...
                    DocumentRewriteSessionType type = (selection.y == 0 && document.getLength() > 1000)
                            || selection.y > 1000 ? DocumentRewriteSessionType.SEQUENTIAL
                    ...
                    try
                    {
                        if (fContentFormatter instanceof IContentFormatterExtension)
                        {
                            ...
                            else
                            {
                                ...
                                context.setProperty(FormattingContextProperties.CONTEXT_REGION, new Region(selection.x,
                                        selection.y));
                            }
    ...
    /**
     * Updates all slave documents of the given document. This default implementation calls
     * <code>updateSlaveDocument</code> for their current visible range. Subclasses may reimplement.
     * 
