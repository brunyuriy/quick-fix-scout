...
/**
 * This manager controls the layout, content, and visibility of an information control in reaction to mouse hover events
 * issued by the vertical ruler of a source viewer.
 * 
 * @since 2.0
 */
public class AnnotationBarHoverManager extends AbstractHoverInformationControlManager
{
     ...
     * pointer leaves the subject area, a mouse button is pressed, the user presses a key, or the subject control is
     * resized or moved.
     * 
     * @since 3.0
     * @deprecated As of 3.4, no longer used as closer from super class is used
     */
    protected class Closer extends MouseTrackAdapter implements IInformationControlCloser, MouseListener,
            MouseMoveListener, ControlListener, KeyListener, DisposeListener, ShellListener, Listener
    {
        ...
        /**
         * Stops the information control and if <code>delayRestart</code> is set allows restart only after a certain
         * delay.
         * 
    ...
    /**
     * Creates an annotation hover manager with the given parameters. In addition, the hovers anchor is RIGHT and the
     * margin is 5 points to the right.
     * 
     ...
     *             {@link AnnotationBarHoverManager#AnnotationBarHoverManager(IVerticalRulerInfo, ISourceViewer, IAnnotationHover, IInformationControlCreator)}
     */
    public AnnotationBarHoverManager(ISourceViewer sourceViewer, IVerticalRuler ruler,
            IAnnotationHover annotationHover, IInformationControlCreator creator)
    {
     ...
     * @param sourceViewer the source viewer this manager connects to
     * @param annotationHover the annotation hover providing the information to be displayed or <code>null</code> if
     *            none
     * @param creator the information control creator
     * @since 2.1
     */
    public AnnotationBarHoverManager(IVerticalRulerInfo rulerInfo, ISourceViewer sourceViewer,
            IAnnotationHover annotationHover, IInformationControlCreator creator)
    {
        super(creator);
    ...
    protected void computeInformation()
    {
        ...
        if (hover instanceof IAnnotationHoverExtension)
        {
            ...
            if (range != null)
                setInformation(extension.getHoverInfo(fSourceViewer, range, computeNumberOfVisibleLines()),
                        computeArea(range));
            else
                setInformation(null, null);
        }
    ...
    /**
     * Adapts a given line range so that the result is a line range that does not overlap with any collapsed region and
     * fits into the view port of the attached viewer.
     * 
    ...
    /**
     * Adapts a given line range so that the result is a line range that does not overlap with any collapsed region of
     * the attached viewer.
     * 
     * @param lineRange the original line range
     * @param line the anchor line
    ...
    /**
     * Adapts a given line range so that the result is a line range that fits into the view port of the attached viewer.
     * 
     * @param lineRange the original line range
     * @return the adapted line range
     * @since 3.0
    ...
    /**
     * Returns the region out of the given set that contains the given line or <code>null</code>.
     * 
    ...
    /**
     * Returns the visible area of the vertical ruler covered by the given line range.
     * 
     * @param lineRange the line range
    ...
    private Rectangle computeArea(ILineRange lineRange)
    {
        try
        {
            ...
            int height = JFaceTextUtil.computeLineHeight(text, startLine, startLine + lineRange.getNumberOfLines(),
                    lineRange.getNumberOfLines());
    ...
    /**
     * Determines the hover to be used to display information based on the source of the mouse hover event. If
     * <code>fVerticalRulerInfo</code> is not a composite ruler, the standard hover is returned.
     * 
    ...
    /*
     * @see
     * org.eclipse.jface.text.AbstractInformationControlManager#computeSizeConstraints(org.eclipse.swt.widgets.Control,
     * org.eclipse.swt.graphics.Rectangle, org.eclipse.jface.text.IInformationControl)
     * @since 3.0
     */
    ...
    protected Point computeLocation(Rectangle subjectArea, Point controlSize, Anchor anchor)
    {
        ...
        if (allowMouseExit)
        {
            ...
            if (anchor == AbstractInformationControlManager.ANCHOR_RIGHT)
                ...
                return subjectControl.toDisplay(subjectArea.x + subjectArea.width - controlSize.x + 4,
                        subjectArea.y - 2);
        }
        fAllowMouseExit = false;
    ...
    /**
     * Returns the currently shown annotation hover or <code>null</code> if none hover is shown.
     * 
