...
/**
 * Abstract implementation of a {@link IVerticalRulerColumn} that uses a {@link Canvas} to draw the ruler contents and
 * which handles scrolling and mouse selection. <h3>Painting</h3> Subclasses can hook into the paint loop at three
 * levels:
 * <ul>
 * <li>Override <strong>{@link #paint(GC, ILineRange)}</strong> to control the entire painting of the ruler.</li>
 * <li>Override <strong>{@link #paintLine(GC, int, int, int, int)}</strong> to control the painting of a line.</li>
 * <li>Leave the painting to the default implementation, but override <strong>{@link #computeBackground(int)}</strong>,
 * <strong>{@link #computeForeground(int)}</strong> and <strong>{@link #computeText(int)}</strong> to specify the ruler
 * appearance for a line.</li>
 * </ul>
 * <h3>Invalidation</h3> Subclasses may call {@link #redraw()} to mark the entire ruler as needing to be redrawn.
 * Alternatively, use {@link #redraw(ILineRange)} to only invalidate a certain line range, for example due to changes to
 * the display model. <h3>Configuration</h3> Subclasses can set the following properties. Setting them may trigger
 * redrawing.
 * <ul>
 ...
 * @since 3.3
 */
public abstract class AbstractRulerColumn implements IVerticalRulerColumn, IVerticalRulerInfo,
        IVerticalRulerInfoExtension
{
    ...
    private final class InternalListener implements IViewportListener, ITextListener
    {
        ...
        {
            /*
             * Redraw: - when the viewer is drawing, and any of the following - the widget was not full before the
             * change - the widget is not full after the change - the document event was a visual modification (no
             * document event attached) - for example when the projection changes.
             */
            if (!event.getViewerRedrawState())
                return;
            if (fWasShowingEntireContents || event.getDocumentEvent() == null
                    || JFaceTextUtil.isShowingEntireContents(fStyledText))
                redraw();
        }
    }
    ...
    /*
     * Implementation and context of this ruler - created and set in createControl(), disposed of in columnRemoved().
     */
     ...
     * <p>
     * The default implementation returns <code>SWT.NO_BACKGROUND</code>.
     * </p>
     * <p>
    ...
    /**
     * The new width in pixels. The <code>DEFAULT_WIDTH</code> constant specifies the default width.
     * 
    ...
    /**
     * Returns the parent ruler, <code>null</code> before {@link #createControl(CompositeRuler, Composite)} has been
     * called.
     * 
    ...
    /**
     * Sets the default background color for this column. The default background is used as default implementation of
     * {@link #computeBackground(int)} and also to paint the area of the ruler that does not correspond to any lines
     * (when the viewport is not entirely filled with lines).
     * 
     * @param background the default background color, <code>null</code> to use the text widget's background
     */
    ...
    /**
     * Paints the ruler. Note that <code>lines</code> reference widget line indices, and that <code>lines</code> may not
     * cover the entire viewport, but only the lines that need to be painted. The lines may not be entirely visible.
     * <p>
     * Subclasses may replace or extend. The default implementation calls {@link #paintLine(GC, int, int, int, int)} for
     * every visible line.
     * </p>
     * 
     ...
     * <p>
     * Subclasses may replace or extend. The default implementation draws the text obtained by {@link #computeText(int)}
     * in the {@link #computeForeground(int) foreground color} and fills the entire width using the
     * {@link #computeBackground(int) background color}. The text is drawn {@link #getTextInset()} pixels to the right
     * of the left border.
     * </p>
     * 
     * @param gc the graphics context to paint on
    ...
    /**
     * Returns the text to be drawn for a certain line by {@link #paintLine(GC, int, int, int, int)}, <code>null</code>
     * for no text. The default implementation returns <code>null</code>.
     * <p>
    ...
    /**
     * Returns the background color drawn for a certain line by {@link #paintLine(GC, int, int, int, int)}. The default
     * implementation returns {@link #getDefaultBackground()}.
     * <p>
     * Subclasses may replace or extend.
     * </p>
     * 
    ...
    /**
     * Returns the foreground color drawn for a certain line by {@link #paintLine(GC, int, int, int, int)}. The default
     * implementation returns a {@link SWT#COLOR_DARK_GRAY} color.
     * <p>
     * Subclasses may replace or extend.
     * </p>
     * 
    ...
    /**
     * Scrolls the canvas vertically (adapted from {@linkplain StyledText StyledText.scrollVertical()}).
     * 
     * @param pixels the number of pixels to scroll (negative to scroll upwards)
     * @return <code>true</code> if the widget was scrolled, <code>false</code> if the widget was not scrolled
     */
