 ...
 * </ul>
 * It is assumed that range indication and visual annotations are shown inside the same presentation area. There are no
 * assumptions about whether this area is different from the viewer's text widget.
 * <p>
 * As the visibility of visual annotations can dynamically be changed, it is assumed that the annotation presentation
 * area can dynamically be hidden if it is different from the text widget.
 * <p>
 * In order to provide backward compatibility for clients of <code>ISourceViewer</code>, extension interfaces are used
 * as a means of evolution. The following extension interfaces exist:
 * <ul>
 * <li>{@link org.eclipse.jface.text.source.ISourceViewerExtension} since version 2.1 introducing the concept of an
 * annotation overview.</li>
 * <li>{@link org.eclipse.jface.text.source.ISourceViewerExtension2} since version 3.0 allowing source viewers to roll
 * back a previously performed configuration and allows access to the viewer's visual annotation model.</li>
 * <li>{@link org.eclipse.jface.text.source.ISourceViewerExtension3} since version 3.2 introducing the concept of a
 * quick assist assistant and providing access to the quick assist invocation context as well as the current annotation
 * hover.</li>
 * <li>{@link org.eclipse.jface.text.source.ISourceViewerExtension4} since version 3.4 introducing API to access a
 * minimal set of content assistant APIs.</li>
 * </ul>
 * </p>
 * <p>
...
public interface ISourceViewer extends ITextViewer
{
    ...
    /**
     * Text operation code for requesting content assist to show the content information for the current insert
     * position.
     */
    int CONTENTASSIST_CONTEXT_INFORMATION = ITextOperationTarget.STRIP_PREFIX + 2;
    /**
     * Text operation code for formatting the selected text or complete document of this viewer if the selection is
     * empty.
     */
     ...
     */
    /**
     * Text operation code for requesting quick assist. This will normally show quick assist and quick fix proposals for
     ...
     * Since 3.0 this method can be called again after a call to {@link ISourceViewerExtension2#unconfigure()}.
     * 
    ...
    /**
     * Sets the annotation hover of this source viewer. The annotation hover provides the information to be displayed in
     * a hover popup window if requested over the annotation presentation area. The annotation hover is assumed to be
     * line oriented.
     * 
    ...
    /**
     * Sets the given document as this viewer's text model and the given annotation model as the model for this viewer's
     * visual annotations. The presentation is accordingly updated. An appropriate <code>TextEvent</code> is issued.
     * This text event does not carry a related document event.
     * 
    ...
    /**
     * Sets the given document as this viewer's text model and the given annotation model as the model for this viewer's
     * visual annotations. The presentation is accordingly updated whereby only the specified region is exposed. An
     * appropriate <code>TextEvent</code> is issued. The text event does not carry a related document event. This method
     * is a convenience method for <code>setDocument(document, annotationModel);setVisibleRegion(offset, length)</code>.
     * 
    ...
    /**
     * Returns this viewer's annotation model. Use {@link ISourceViewerExtension2#getVisualAnnotationModel()}in order to
    ...
    /**
     * Sets the annotation used by this viewer as range indicator. The range covered by this annotation is referred to
     * as range indication.
     * 
    ...
    /**
     * Sets the viewers's range indication to the specified range. It is indicated whether the cursor should also be
     * moved to the beginning of the specified range.
     * 
    ...
    /**
     * Controls the visibility of annotations and in the case of separate presentation areas of text and annotations,
     * the visibility of the annotation's presentation area.
     * <p>
