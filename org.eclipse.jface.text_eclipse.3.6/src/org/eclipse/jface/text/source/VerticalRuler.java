...
/**
 * A vertical ruler which is connected to a text viewer. Single column standard implementation of
 * {@link org.eclipse.jface.text.source.IVerticalRuler}.
 * <p>
 * The same can be achieved by using <code>CompositeRuler</code> configured with an <code>AnnotationRulerColumn</code>.
...
public final class VerticalRuler implements IVerticalRuler, IVerticalRulerExtension
{
    ...
    /**
     * Returns the document offset of the upper left corner of the widgets view port, possibly including partially
     * visible lines.
     * 
    ...
    /**
     * Draws the vertical ruler w/o drawing the Canvas background. Uses <code>ITextViewerExtension5</code> for its
    ...
    protected void doPaint1(GC gc)
    {
        ...
        for (int layer = 0; layer < maxLayer; layer++)
        {
            ...
            while (iter.hasNext())
            {
                ...
                if (position == null)
                    continue;
                IRegion widgetRegion = extension.modelRange2WidgetRange(new Region(position.getOffset(), position
                        .getLength()));
