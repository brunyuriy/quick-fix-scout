 ...
 * Implementers of this interface have to define the presentation modus. This can either depend on the connected
 * viewer's view port or not. If the modus is view port dependent the ruler only shows those annotations that are
 * attached to document regions that are visible in the view port. If independent, the presented annotations can also be
 * attached to invisible document regions. This interfaces comprises three contracts:
 * <ul>
 ...
 * coordinates of the ruler's control and line numbers based on the connected text viewer's document (see
 * {@link org.eclipse.jface.text.source.IVerticalRulerInfo}).
 * </ul>
 * <p>
 * In order to provide backward compatibility for clients of <code>IVerticalRuler</code>, extension interfaces are used
 * as a means of evolution. The following extension interfaces exist:
 * <ul>
 * <li>{@link org.eclipse.jface.text.source.IVerticalRulerExtension} since version 2.0 introducing setters for font and
 * mouse button activity location.</li>
 * </ul>
 * </p>
 * <p>
 * Clients may implement this interface or use the default implementation provided by
 * {@link org.eclipse.jface.text.source.CompositeRuler} and {@link org.eclipse.jface.text.source.VerticalRuler}.
 * </p>
 * 
...
{
    /**
     * Associates an annotation model with this ruler. A value <code>null</code> is acceptable and clears the ruler.
     * 
    ...
    /**
     * Returns the current annotation model of this ruler or <code>null</code> if the ruler has no model.
     * 
