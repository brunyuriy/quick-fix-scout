...
/**
 * Annotation model for visual annotations. Assume a viewer's input element is annotated with some semantic annotation
 * such as a breakpoint and that it is simultaneously shown in multiple viewers. A source viewer, e.g., supports visual
 * range indication for which it utilizes annotations. The range indicating annotation is specific to the visual
 * presentation of the input element in this viewer and thus should only be visible in this viewer. The breakpoints
 * however are independent from the input element's presentation and thus should be shown in all viewers in which the
 * element is shown. As a viewer supports one vertical ruler which is based on one annotation model, there must be a
 * visual annotation model for each viewer which all wrap the same element specific model annotation model.
 */
class VisualAnnotationModel extends AnnotationModel implements IAnnotationModelListener
{
    ...
    /**
     * Constructs a visual annotation model which wraps the given model based annotation model
     * 
