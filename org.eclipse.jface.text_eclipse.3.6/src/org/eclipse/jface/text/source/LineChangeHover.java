...
public class LineChangeHover implements IAnnotationHover, IAnnotationHoverExtension, IInformationProviderExtension2
{
    ...
    /**
     * Formats the source w/ syntax coloring etc. This implementation replaces tabs with spaces. May be overridden by
     * subclasses.
     * 
    ...
    /**
     * Returns a replacement for the tab character. The default implementation returns a tabulator character, but
     * subclasses may override to specify a number of spaces.
     * 
     * @return a whitespace String that will be substituted for the tabulator character
     */
    ...
    /**
     * Computes the content of the hover for the document contained in <code>viewer</code> on line <code>line</code>.
     * 
    ...
    /**
     * Takes a list of <code>ILineDiffInfo</code>s and computes a hover of at most <code>maxLines</code>. Added lines
     * are prefixed with a <code>'+'</code>, changed lines with <code>'>'</code> and deleted lines with <code>'-'</code>
     * .
     * <p>
     * Deleted and added lines can even each other out, so that a number of deleted lines get displayed where - in the
     * current document - the added lines are.
     * 
     * @param diffInfos a <code>List</code> of <code>ILineDiffInfo</code>
     * @param maxLines the maximum number of lines. Note that adding up all annotations might give more than that due to
     *            deleted lines.
    ...
    {
        /*
         * maxLines controls the size of the hover (not more than what fits into the display are of the viewer). added
         * controls how many lines are added - added lines are
         */
    ...
    private ILineDiffer getDiffer(ISourceViewer viewer)
    {
        ...
        {
            IAnnotationModel diffModel = ((IAnnotationModelExtension) model)
     ...
     * @return the selection in the document displayed in <code>viewer</code> containing <code>line</code> that is
     *         covered by the hover information returned by the receiver.
     */
    protected Point computeLineRange(ISourceViewer viewer, int line, int min, int max)
    {
        /*
         * Algorithm: All lines that have changes to themselves (added, changed) are taken that form a contiguous block
         * of lines that includes <code>line</code>. If <code>line</code> is itself unchanged, if there is a deleted
         * line either above or below, or both, the lines +/- 1 from <code>line</code> are included in the search as
         * well, without applying this last rule to them, though. (I.e., if <code>line</code> is unchanged, but has a
         * deleted line above, this one is taken in. If the line above has changes, the block is extended from there. If
         * the line has no changes itself, the search stops). The block never extends the visible line range of the
         * viewer.
         */
        ILineDiffer differ = getDiffer(viewer);
        if (differ == null)
        ...
        while (l >= min && info != null
                && (info.getChangeType() == ILineDiffInfo.CHANGED || info.getChangeType() == ILineDiffInfo.ADDED))
        {
     ...
     * @return <code>startLine - 1</code> if that line exists and is an unchanged line followed by deletions,
     *         <code>startLine</code> otherwise
     */
     ...
     * @return <code>lastLine - 1</code> if that line exists and is an unchanged line followed by deletions,
     *         <code>startLine</code> otherwise
     */
    ...
    /*
     * @see
     * org.eclipse.jface.text.source.IAnnotationHoverExtension#getHoverLineRange(org.eclipse.jface.text.source.ISourceViewer
     * , int)
     */
