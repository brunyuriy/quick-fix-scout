...
/**
 * An information control manager that shows information in response to mouse hover events. The mouse hover events are
 * caught by registering a {@link org.eclipse.swt.events.MouseTrackListener} on the manager's subject control. The
 * manager has by default an information control closer that closes the information control as soon as the mouse pointer
 * leaves the subject area, the user presses a key, or the subject control is resized, moved, or deactivated.
 * <p>
 * When being activated by a mouse hover event, the manager disables itself, until the mouse leaves the subject area.
 * Thus, the manager is usually still disabled, when the information control has already been closed by the closer.
 * 
...
{
    /**
     * The information control closer for the hover information. Closes the information control as soon as the mouse
     * pointer leaves the subject area (unless "move into hover" is enabled), a mouse button is pressed, the user
     * presses a key, or the subject control is resized, moved, or loses focus.
     */
    class Closer implements IInformationControlCloser, MouseListener, MouseMoveListener, ControlListener, KeyListener,
            SelectionListener, Listener
    {
                    ...
                    else if (!isReplaceInProgress())
                    {
                        ...
                        if (event.widget instanceof Control && infoControl instanceof IInformationControlExtension5)
                        {
                            ...
                            else if (cancelReplacingDelay())
                            {
                                ...
                                {
                                    stop(); // avoid that someone else replaces the info control before the async is
                                            // exec'd
                                    if (infoControl instanceof IDelayedInputChangeProvider)
                                    {
                                        ...
                                        final IInputChangedListener inputChangeListener = new DelayedInputChangeListener(
                                                delayedICP, getInformationControlReplacer());
        ...
        private void handleMouseMove(Event event)
        {
            ...
            {
                /*
                 * Use inKeepUpZone() to make sure it also works when the hover is completely outside of the subject
                 * control.
                 */
     ...
     * <ul>
     * <li>start function: initiates the computation of the information to be presented. This happens on receipt of a
     * mouse hover event and disables the information control manager,
     * <li>restart function: tracks mouse move and shell activation event to determine when the information control
     * manager needs to be reactivated.
     * </ul>
     */
    class MouseTracker extends ShellAdapter implements MouseTrackListener, MouseMoveListener
    {
        ...
        /**
         * Sets this mouse tracker's subject area, the area to be tracked in order to re-enable the information control
         * manager.
         * 
        ...
        /**
         * Starts this mouse tracker. The given control becomes this tracker's subject control. Installs itself as mouse
         * track listener on the subject control.
         * 
        ...
        /**
         * Stops this mouse tracker. Removes itself as mouse track, mouse move, and shell listener from the subject
         * control.
         */
        public void stop()
        {
        ...
        /**
         * Initiates the computation of the information to be presented. Sets the initial hover area to a small
         * rectangle around the hover event location. Adds mouse move and shell activation listeners to track whether
         * the computed information is, after completion, useful for presentation and to implement the restart function.
         * 
        ...
        public void mouseHover(MouseEvent event)
        {
            if (fIsComputing
                    || fIsInRestartMode
                    || (fSubjectControl != null && !fSubjectControl.isDisposed() && fSubjectControl.getShell() != fSubjectControl
                            .getShell().getDisplay().getActiveShell()))
            {
                if (DEBUG)
                    System.out
                            .println("AbstractHoverInformationControlManager...mouseHover: @ " + event.x + "/" + event.y + " : hover cancelled: fIsComputing= " + fIsComputing + ", fIsInRestartMode= " + fIsInRestartMode); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
                return;
            }
        ...
        /**
         * Deactivates this tracker's restart function and enables the information control manager. Does not have any
         * effect if the tracker is still executing the start function (i.e. computing the information to be presented.
         */
        ...
        /**
         * Determines whether the computed information is still useful for presentation. This is not the case, if the
         * shell of the subject control has been deactivated, the mouse left the subject control, or the mouse moved on,
         * so that it is no longer in the subject area.
         * 
    ...
    /**
     * The delay in {@link ITextViewerExtension8.EnrichMode#AFTER_DELAY} mode after which the hover is enriched when the
     * mouse has stopped moving inside the hover.
     * 
     * @since 3.4
     */
    ...
    /**
     * Indicates whether we have received a MouseDown event and are waiting for a MouseUp (and don't replace the
     * information control until that happened).
     * 
     * @since 3.4
     */
    ...
    /**
     * Creates a new hover information control manager using the given information control creator. By default a
     * <code>Closer</code> instance is set as this manager's closer.
     * 
    ...
    /**
     * Tests whether a given mouse location is within the keep-up zone. The hover should not be hidden as long as the
     * mouse stays inside this zone.
     * 
     ...
     * @param blowUp If <code>true</code>, then calculate for the closer, i.e. blow up the keepUp area. If
     *            <code>false</code>, then use tight bounds for hover detection.
    ...
    private boolean inKeepUpZone(int x, int y, Control subjectControl, Rectangle subjectArea, boolean blowUp)
    {
        if (subjectArea.contains(x, y))
            return true;
        IInformationControl iControl = getCurrentInformationControl();
        if ((iControl instanceof IInformationControlExtension5 && !((IInformationControlExtension5) iControl)
                .isVisible()))
        {
            iControl = null;
    ...
    /**
     * Starts replacing the information control, considering the current {@link ITextViewerExtension8.EnrichMode}. If
     * set to {@link ITextViewerExtension8.EnrichMode#AFTER_DELAY}, this method cancels previous requests and restarts
     * the delay timer.
     * 
     * @param display the display to be used for the call to {@link #replaceInformationControl(boolean)} in the UI
     *            thread
     */
    private void startReplaceInformationControl(final Display display)
    {
        ...
        fReplacingDelayJob = new Job("AbstractHoverInformationControlManager Replace Delayer") { //$NON-NLS-1$
            ...
            {
                if (monitor.isCanceled() || display.isDisposed()) { return Status.CANCEL_STATUS; }
