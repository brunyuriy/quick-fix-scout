...
/**
 * A generic scanner which can be "programmed" with a sequence of rules. The scanner is used to get the next token by
 * evaluating its rule in sequence until one is successful. If a rule returns a token which is undefined, the scanner
 * will proceed to the next rule. Otherwise the token provided by the rule will be returned by the scanner. If no rule
 * returned a defined token, this scanner returns a token which returns <code>true</code> when calling
 * <code>isOther</code>, unless the end of the file is reached. In this case the token returns <code>true</code> when
 * calling <code>isEOF</code>.
 * 
 * @see IRule
 */
public class RuleBasedScanner implements ICharacterScanner, ITokenScanner
{
    ...
    /**
     * Configures the scanner's default return token. This is the token which is returned when none of the rules fired
    ...
    /**
     * Checks that the given range is valid. See https://bugs.eclipse.org/bugs/show_bug.cgi?id=69292
     * 
