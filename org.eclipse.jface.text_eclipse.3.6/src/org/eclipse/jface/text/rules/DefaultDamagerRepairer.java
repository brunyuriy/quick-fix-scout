...
/**
 * A standard implementation of a syntax driven presentation damager and presentation repairer. It uses a token scanner
 * to scan the document and to determine its damage and new text presentation. The tokens returned by the scanner are
 * supposed to return text attributes as their data.
 * 
 * @see ITokenScanner
 * @since 2.0
 */
public class DefaultDamagerRepairer implements IPresentationDamager, IPresentationRepairer
{
    ...
    /**
     * Creates a damager/repairer that uses the given scanner and returns the given default text attribute if the
     * current token does not carry a text attribute.
     * 
     * @param scanner the token scanner to be used
     * @param defaultTextAttribute the text attribute to be returned if non is specified by the current token, may not
     *            be <code>null</code>
    ...
    /**
     * Creates a damager/repairer that uses the given scanner. The scanner may not be <code>null</code> and is assumed
     * to return only token that carry text attributes.
     * 
     ...
     * </p>
     * 
     * @return the full lines containing the document changes described by the document event, clipped by the given
     *         partition. If there was a partitioning change then the whole partition is returned.
     */
     ...
     * attribute it is assumed that it is the encoded text attribute. It returns the default text attribute if there is
     * no encoded text attribute found.
     * 
