...
/**
 * A standard implementation of a syntax driven document partitioner. It uses a rule based scanner to scan the document
 * and to determine the document's partitioning. The tokens returned by the rules the scanner is configured with are
 * supposed to return the partition type as their data. The partitioner remembers the document's partitions in the
 * document itself rather than maintaining its own data structure.
 * 
 * @see IRule
        ...
        IDocumentPartitionerExtension2
{
    ...
    public IRegion documentChanged2(DocumentEvent e)
    {
        try
        {
            ...
            {
                /*
                 * determine character position at which the scanner starts: first position behind the last non-default
                 * partition the actual position is not involved with
                 */
                ...
                do
                {
                    ...
                    p = category[first];
                        ...
                        || (e.getOffset() == fPreviousDocumentLength && (p.getOffset() + p.getLength() == fPreviousDocumentLength)));
            ...
            while (!token.isEOF())
            {
                ...
                while (first < category.length)
                {
                    ...
                    if (lastScannedPosition >= p.offset + p.length
                            || (p.overlapsWith(start, length) && (!d.containsPosition(fPositionCategory, start, length) || !contentType
                                    .equals(p.getType()))))
                    {
        ...
        try
        {
            ...
            if (previous != null)
            {
                ...
                if ((includeZeroLengthPartitions || gap.getLength() > 0)
                        && ((includeZeroLengthPartitions && offset + length == gapOffset && gap.length == 0) || gap
