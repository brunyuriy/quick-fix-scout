...
/**
 * Standard implementation of <code>IPredicateRule</code>. Is is capable of detecting a pattern which begins with a
 * given start sequence and ends with a given end sequence. If the end sequence is not specified, it can be either end
 * of line, end or file, or both. Additionally, the pattern can be constrained to begin in a certain column. The rule
 * can also be used to check whether the text to scan covers half of the pattern, i.e. contains the end sequence
...
public class PatternRule implements IPredicateRule
{
    ...
    /**
     * Creates a rule for the given starting and ending sequence. When these sequences are detected the rule will return
     * the specified token. Alternatively, the sequence can also be ended by the end of the line or the end of the file.
     ...
     * Creates a rule for the given starting and ending sequence. When these sequences are detected the rule will return
     * the specified token. Alternatively, the sequence can also be ended by the end of the line or the end of the file.
     * Any character which follows the given escapeCharacter will be ignored. An end of line immediately after the given
     * <code>lineContinuationCharacter</code> will not cause the pattern to terminate even if <code>breakOnEOL</code> is
     * set to true.
     * 
     * @param startSequence the pattern's start sequence
     * @param endSequence the pattern's end sequence, <code>null</code> is a legal value
     ...
     * @param breaksOnEOL indicates whether the end of the line also terminates the pattern
     * @param breaksOnEOF indicates whether the end of the file also terminates the pattern
     * @param escapeContinuesLine indicates whether the specified escape character is used for line continuation, so
     *            that an end of line immediately after the escape character does not terminate the pattern, even if
     ...
     * result of this method is <code>true</code> if the rule breaks on the end of the line, or if the EOF character is
     * read.
     * 
     * @param scanner the character scanner to be used
