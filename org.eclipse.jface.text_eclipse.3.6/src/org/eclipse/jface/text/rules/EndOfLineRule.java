...
/**
 * A specific configuration of a single line rule whereby the pattern begins with a specific sequence but is only ended
 * by a line delimiter.
 */
public class EndOfLineRule extends SingleLineRule
{
    ...
    /**
     * Creates a rule for the given starting sequence which, if detected, will return the specified token. Any character
     * which follows the given escape character will be ignored.
     * 
     * @param startSequence the pattern's start sequence
     * @param token the token to be returned on success
    ...
    /**
     * Creates a rule for the given starting sequence which, if detected, will return the specified token. Any character
     * which follows the given escape character will be ignored. In addition, an escape character immediately before an
     ...
     *            that an end of line immediately after the escape character does not terminate the line, even if
