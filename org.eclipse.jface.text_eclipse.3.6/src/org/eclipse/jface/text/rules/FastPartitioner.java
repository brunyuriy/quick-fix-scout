...
/**
 * A standard implementation of a document partitioner. It uses an {@link IPartitionTokenScanner} to scan the document
 * and to determine the document's partitioning. The tokens returned by the scanner must return the partition type as
 * their data. The partitioner remembers the document's partitions in the document itself rather than maintaining its
 * own data structure.
 * <p>
 * To reduce array creations in {@link IDocument#getPositions(String)}, the positions get cached. The cache is cleared
 * after updating the positions in {@link #documentChanged2(DocumentEvent)}. Subclasses need to call
 * {@link #clearPositionCache()} after modifying the partitioner's positions. The cached positions may be accessed
 * through {@link #getPositions()}.
 * </p>
 * 
        ...
        IDocumentPartitionerExtension2, IDocumentPartitionerExtension3
{
    ...
    /*
     * @see org.eclipse.jface.text.IDocumentPartitionerExtension3#startRewriteSession(org.eclipse.jface.text.
