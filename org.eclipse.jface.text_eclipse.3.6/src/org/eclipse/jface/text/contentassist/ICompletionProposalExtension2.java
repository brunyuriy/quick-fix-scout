...
{
    /**
     * Applies the proposed completion to the given document. The insertion has been triggered by entering the given
     * character with a modifier at the given offset. This method assumes that
     * {@link #validate(IDocument, int, DocumentEvent)} returns <code>true</code> if called for <code>offset</code>.
     * 
    ...
    /**
     * Requests the proposal to be validated with respect to the document event. If the proposal cannot be validated,
     * the methods returns <code>false</code>. If the document event was <code>null</code>, only the caret offset was
     * changed, but not the document. This method replaces
     * {@link ICompletionProposalExtension#isValidFor(IDocument, int)}
     * 
