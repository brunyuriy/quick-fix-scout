...
class CompletionProposalPopup implements IContentAssistListener
{
    ...
    /**
     * The content assist subject control adapter. This replaces <code>fViewer</code>
     * 
     * @since 3.0
     */
    ...
    /**
     * <code>true</code> if <code>fFilterRunnable</code> has been posted, <code>false</code> if not.
     * 
     * @since 3.1.1
     */
    private boolean fIsFilterPending = false;
    /**
     * The info message at the bottom of the popup, or <code>null</code> for no popup (if ContentAssistant does not
     * provide one).
     * 
     * @since 3.2
     */
    ...
    /**
     * Tells whether colored labels support is enabled. Only valid while the popup is active.
     * 
     * @since 3.4
     */
    ...
    /**
     * Hides the popup and returns <code>true</code> if the popup is configured to never display an empty list. Returns
    ...
    /**
     * If content assist is set up to handle cycling, then the proposals are recomputed. Otherwise, nothing happens.
     * 
     * @since 3.2
     */
    ...
    private void createProposalSelector()
    {
        ...
        // XXX Kivanc starts here...
        if (speculationHack_ != null)
            speculationHack_.proposalTableSet(fProposalTable, this);
        if (observerHack_ != null)
            observerHack_.proposalTableSet();
        // XXX Kivanc ends here...
    }

    /**
     * Returns the minimal required height for the proposal, may return 0 if the popup has not been created yet.
     * 
    ...
    {
        /*
         * Make sure that there is no filter runnable pending. See https://bugs.eclipse.org/bugs/show_bug.cgi?id=31427
         */
        if (fIsFilterPending)
            fFilterRunnable.run();
        ...
        if (speculationHack_ != null)
            ...
            result = (ICompletionProposal) speculationHack_.getSelectedProposal(i);
        /*
         * Note: result that comes from speculationHack might be null. This is due to the fact that
         * CompletionProposalPopup is used for things other than quick fix (i.e., auto-completion). Moreover, for these
         * things I don't have yet a hook or something to interact with the CompletionProposalPopupCoordinatinator, so I
         * return 'null'.
         */
        if (result == null)
        {
            /*
             * XXX Kivanc changed location of this block: if was after: if (!Helper.okToUse(fProposalTable)) return
             * null; This blocks position must be changed since if I have calculated the augmented proposal, with the
             * addition of best proposals, the selected inside may be out of the array indices. It we check this before,
             * then it automatically returns null.
             */
            if (fFilteredProposals == null || i < 0 || i >= fFilteredProposals.length)
                return null;
            // XXX Kivanc changed location of this block: if was after: if (!Helper.okToUse(fProposalTable)) return
            // null;
            result = fFilteredProposals[i];
        }
        return result;
    ...
    private void insertSelectedProposalWithMask(int stateMask)
    {
        ...
        // XXX Kivanc starts here...
        /*
         * hide() is the method that kills the popup, and this is also recorded as an event. So, we actually need to
         * record the proposal that is selected to be applied even before it is applied. Otherwise, Eclipse first closes
         * the popup and then applies the proposal and it is harder to detect if the user has selected a proposal or
         * not.
         */
        if (speculationHack_ != null)
            speculationHack_.proposalSelected(p);
        if (observerHack_ != null)
            observerHack_.proposalSelected(p);
        // XXX Kivanc ends here...
        hide();
        if (p != null)
    ...
    public void hide()
    {
        ...
        // XXX Kivanc starts here...
        if (speculationHack_ != null)
            speculationHack_.popupClosed();
        if (observerHack_ != null)
            observerHack_.popupClosed();
        // XXX Kivanc ends here...
    }
     ...
     * @param isFilteredSubset if <code>true</code>, the proposal table is not cleared, but the proposals that are not
     *            in the passed array are removed from the displayed set
     */
        ...
        if (fContentAssistant.addContentAssistListener(this, ContentAssistant.PROPOSAL_SELECTOR))
        {
            ...
            if (fFocusHelper == null)
            {
                fFocusHelper = new IEditingSupport()
                {
                    public boolean isOriginator(DocumentEvent event, IRegion focus)
                    {
                        return false; // this helper just covers the focus change to the proposal shell, no remote
                                      // editions
                    }
    ...
    private boolean completeCommonPrefix()
    {
        ...
        for (int i = 0; i < fFilteredProposals.length; i++)
        {
            ...
            if (!(proposal instanceof ICompletionProposalExtension3))
                return false;
            int start = ((ICompletionProposalExtension3) proposal).getPrefixCompletionStart(
                    fContentAssistSubjectControlAdapter.getDocument(), fFilterOffset);
            CharSequence insertion = ((ICompletionProposalExtension3) proposal).getPrefixCompletionText(
                    fContentAssistSubjectControlAdapter.getDocument(), fFilterOffset);
            if (insertion == null)
    ...
    /**
     * Sets the message for the repetition affordance text at the bottom of the proposal. Only has an effect if
     * {@link ContentAssistant#isRepeatedInvocationMode()} returns <code>true</code>.
     * 
     ...
     * 
     * @param above <code>true</code> if the location of the popup is above the caret line, <code>false</code> if it is
     *            below
     * @since 3.3
     */
    ...
    // XXX Kivanc starts here...
    private static ICompletionProposalPopupSpeculationHack getSpeculationHack()
    {
        /*
         * I turned to lazy initialization.
         */
        if (speculationHack_ == null)
            speculationHack_ = SpeculationHackActionManager.getInstance().getCorrectionProposalPopupHandler();
        return speculationHack_;
    }

    private static ICompletionProposalPopupUsageHack getObserverHack()
    {
        /*
         * I turned to lazy initialization.
         */
        if (observerHack_ == null)
            observerHack_ = ObserverHackActionManager.getInstance().getCorrectionProposalPopupHandler();
        return observerHack_;
    }
    // XXX Kivanc ends here...
}
