...
/**
 * Describes the presentation styles for a section of an indexed text such as a document or string. A text presentation
 * defines a default style for the whole section and in addition style differences for individual subsections. Text
 * presentations can be narrowed down to a particular result window. All methods are result window aware, i.e. ranges
 * outside the result window are always ignored.
 * <p>
 * All iterators provided by a text presentation assume that they enumerate non overlapping, consecutive ranges inside
 * the default range. Thus, all these iterators do not include the default range. The default style range must be
...
public class TextPresentation
{
    ...
    class FilterIterator implements Iterator
    {
        ...
        /**
         * <code>skipDefaults</code> tells the enumeration to skip all those style ranges which define the same style as
         * the presentation's default style range.
         * 
    ...
    /**
     * Creates a new empty text presentation. <code>sizeHint</code> tells the expected size of this presentation.
     * 
    ...
    /**
     * Creates a new empty text presentation with the given extent. <code>sizeHint</code> tells the expected size of
     * this presentation.
     * 
    ...
    /**
     * Sets the result window for this presentation. When dealing with this presentation all ranges which are outside
     * the result window are ignored. For example, the size of the presentation is 0 when there is no range inside the
     * window even if there are ranges outside the window. All methods are aware of the result window.
     * 
    ...
    /**
     * Set the default style range of this presentation. The default style range defines the overall area covered by
     * this presentation and its style information.
     * 
    ...
    /**
     * Replaces the given ranges in this presentation. Each range must be a subrange of the presentation's default
     * range. The ranges must be ordered by increasing offset and must not overlap (but may be adjacent).
     * 
    ...
    /**
     * Merges the given ranges into this presentation. Each range must be a subrange of the presentation's default
     * range. The ranges must be ordered by increasing offset and must not overlap (but may be adjacent).
     * 
     * @param ranges the ranges to be added
     * @since 3.0
    ...
    /**
     * Applies the given ranges to this presentation. Each range must be a subrange of the presentation's default range.
     * The ranges must be ordered by increasing offset and must not overlap (but may be adjacent).
     * 
     * @param ranges the ranges to be added
     * @param merge <code>true</code> if the style should be merged instead of replaced
     * @since 3.0
    ...
    /**
     * Returns the index of the first range which comes after the specified window and does not overlap with this
     * window.
     * 
     * @param window the window to be used for searching
    ...
    /**
     * Returns a style range which is relative to the specified window and appropriately clipped if necessary. The
     * original style range is not modified.
     * 
    ...
    /**
     * Returns an iterator which enumerates all style ranged which define a style different from the presentation's
     * default style range. The default style range is not enumerated.
     * 
    ...
    /**
     * Returns an iterator which enumerates all style ranges of this presentation except the default style range. The
     * returned <code>StyleRange</code>s are relative to the start of the presentation's result window.
     * 
     * @return a style range iterator
     */
    ...
    /**
     * Returns the style range with the smallest offset ignoring the default style range or null if the presentation is
     * empty.
     * 
