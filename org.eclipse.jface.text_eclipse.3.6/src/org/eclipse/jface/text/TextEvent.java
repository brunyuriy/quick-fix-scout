...
/**
 * This event is sent to implementers of {@link org.eclipse.jface.text.ITextListener}. It represents a change applied to
 * text viewer. The change is specified as a replace command using offset, length, inserted text, and replaced text. The
 * text viewer issues a text event after the viewer has been changed either in response to a change of the viewer's
 * document or when the viewer's visual content has been changed. In the first case, the text event also carries the
 * original document event. Depending on the viewer's presentation mode, the text event coordinates are different from
 * the document event's coordinates.
 * <p>
