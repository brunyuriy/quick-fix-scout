...
/**
 * A reconcile step is one of several steps of a {@linkplain org.eclipse.jface.text.reconciler.IReconcilingStrategy
 * reconcile strategy} that consists of several steps. This relationship is not coded into an interface but should be
 * used by clients who's reconcile strategy consists of several steps.
 * <p>
 * If a reconcile step has an {@linkplain org.eclipse.jface.text.reconciler.IReconcilableModel input model} it will
 * compute the correct model for the next step in the chain and set the next steps input model before
 * <code>reconcile</code> gets called on that next step. After the last step has reconciled the
 * {@linkplain org.eclipse.jface.text.reconciler.IReconcileResult reconcile result} array gets returned to the previous
 * step. Each step in the chain adapts the result to its input model and returns it to its previous step.
 * </p>
 * <p>
 * Example: Assume a strategy consists of steps A, B and C. And the main model is M. The strategy will set M to be A's
 * input model. What will happen is:
 * <ol>
...
public interface IReconcileStep
{
    ...
    /**
     * Activates incremental reconciling of the specified dirty region. As a dirty region might span multiple content
     * types, the segment of the dirty region which should be investigated is also provided to this reconciling
     * strategy. The given regions refer to the document passed into the most recent call of
     * {@link IReconcilingStrategy#setDocument(org.eclipse.jface.text.IDocument)}.
     * 
    ...
    /**
     * Activates non-incremental reconciling. The reconciling strategy is just told that there are changes and that it
     * should reconcile the given partition of the document most recently passed into
     * {@link IReconcilingStrategy#setDocument(org.eclipse.jface.text.IDocument)}.
     * 
    ...
    /**
     * Tells this reconcile step on which model it will work. This method will be called before any other method and can
     * be called multiple times. The regions passed to the other methods always refer to the most recent model passed
     * into this method.
     * 
