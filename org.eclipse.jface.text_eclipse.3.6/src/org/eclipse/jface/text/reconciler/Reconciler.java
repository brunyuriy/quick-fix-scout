...
/**
 * Standard implementation of {@link org.eclipse.jface.text.reconciler.IReconciler}. The reconciler is configured with a
 * set of {@linkplain org.eclipse.jface.text.reconciler.IReconcilingStrategy reconciling strategies} each of which is
...
public class Reconciler extends AbstractReconciler implements IReconcilerExtension
{
    ...
    /**
     * Creates a new reconciler with the following configuration: it is an incremental reconciler with a standard delay
     * of 500 milliseconds. There are no predefined reconciling strategies. The partitioning it uses is the default
     * partitioning {@link IDocumentExtension3#DEFAULT_PARTITIONING}.
     */
    public Reconciler()
    {
        super();
    ...
    /**
     * Registers a given reconciling strategy for a particular content type. If there is already a strategy registered
     * for this type, the new strategy is registered instead of the old one.
     * 
    ...
    /**
     * Computes and returns the partitioning for the given region of the input document of the reconciler's connected
     * text viewer.
     * 
