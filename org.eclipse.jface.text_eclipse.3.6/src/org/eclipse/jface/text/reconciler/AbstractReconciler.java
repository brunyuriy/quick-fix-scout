...
/**
 * Abstract implementation of {@link IReconciler}. The reconciler listens to input document changes as well as changes
 * of the input document of the text viewer it is installed on. Depending on its configuration it manages the received
 * change notifications in a queue folding neighboring or overlapping changes together. The reconciler processes the
 * dirty regions as a background activity after having waited for further changes for the configured duration of time. A
 * reconciler is started using the {@link #install(ITextViewer)} method. As a first step {@link #initialProcess()} is
 * executed in the background. Then, the reconciling thread waits for changes that need to be reconciled. A reconciler
 * can be resumed by calling {@link #forceReconciling()} independent from the existence of actual changes. This
 * mechanism is for subclasses only. It is the clients responsibility to stop a reconciler using its
 * {@link #uninstall()} method. Unstopped reconcilers do not free their resources.
 * <p>
...
abstract public class AbstractReconciler implements IReconciler
{
    ...
    class BackgroundThread extends Thread
    {
        ...
        /**
         * Suspends the caller of this method until this background thread has emptied the dirty region queue.
         */
        ...
        /**
         * The background activity. Waits until there is something in the queue managing the changes that have been
         * applied to the text viewer. Removes the first change from the queue and process it.
         * <p>
    ...
    class Listener implements IDocumentListener, ITextInputListener
    {
        ...
        public void documentChanged(DocumentEvent e)
        {
            ...
            /*
             * The second OR condition handles the case when the document gets changed while still inside
             * initialProcess().
             */
    ...
    /**
     * Processes a dirty region. If the dirty region is <code>null</code> the whole document is consider being dirty.
     * The dirty region is partitioned by the document and each partition is handed over to a reconciling strategy
    ...
    /**
     * Hook called when the document whose contents should be reconciled has been changed, i.e., the input document of
     * the text viewer this reconciler is installed on. Usually, subclasses use this hook to inform all their
     * reconciling strategies about the change.
     * 
    ...
    /**
     * Tells the reconciler how long it should wait for further text changes before activating the appropriate
    ...
    /**
     * Tells the reconciler whether any of the available reconciling strategies is interested in getting detailed dirty
     * region information or just in the fact that the document has been changed. In the second case, the reconciling
     * can not incrementally be pursued.
     * 
     * @param isIncremental indicates whether this reconciler will be configured with incremental reconciling strategies
     * @see DirtyRegion
     ...
     * <p>
     * If this is set to <code>false</code> an {@link UnsupportedOperationException} will be thrown when this
    ...
    private void createDirtyRegion(DocumentEvent e)
    {
        synchronized (fDirtyRegionQueue)
        {
            ...
            {
                // Insert
                fDirtyRegionQueue.addDirtyRegion(new DirtyRegion(e.getOffset(), e.getText().length(),
                        DirtyRegion.INSERT, e.getText()));
            }
    ...
    /**
     * This method is called on startup of the background activity. It is called only once during the life time of the
     * reconciler. Clients may reimplement this method.
     */
