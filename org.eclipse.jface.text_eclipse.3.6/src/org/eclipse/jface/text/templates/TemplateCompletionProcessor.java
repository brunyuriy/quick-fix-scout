...
/**
 * A completion processor that computes template proposals. Subclasses need to provide implementations for
 * {@link #getTemplates(String)}, {@link #getContextType(ITextViewer, IRegion)} and {@link #getImage(Template)}.
 * 
 * @since 3.0
 */
public abstract class TemplateCompletionProcessor implements IContentAssistProcessor
{
     ...
     * @since 3.1
     */
    protected ICompletionProposal createProposal(Template template, TemplateContext context, Region region,
            int relevance)
    {
     ...
     * Creates a new proposal.
     * <p>
     * The default implementation returns an instance of {@link TemplateProposal}. Subclasses may replace this method to
    ...
    /**
     * Creates a concrete template context for the given region in the document. This involves finding out which context
     * type is valid at the given location, and then creating a context of this type. The default implementation returns
     * a <code>DocumentTemplateContext</code> for the context type at the given location.
     * 
     ...
     * @return the context type that can handle template expansion for the given location, or <code>null</code> if none
     *         exists
     */
    protected abstract TemplateContextType getContextType(ITextViewer viewer, IRegion region);

    /**
     * Returns the relevance of a template given a prefix. The default implementation returns a number greater than zero
     * if the template name starts with the prefix, and zero otherwise.
     * 
    ...
    /**
     * Heuristically extracts the prefix used for determining template relevance from the viewer's document. The default
     * implementation returns the String from offset backwards that forms a java identifier.
     * 
