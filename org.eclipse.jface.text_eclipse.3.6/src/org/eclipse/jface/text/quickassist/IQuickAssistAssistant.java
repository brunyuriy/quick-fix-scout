...
/**
 * An <code>IQuickAssistAssistant</code> provides support for quick fixes and quick assists. The quick assist assistant
 * is a {@link ISourceViewer} add-on. Its purpose is to propose, display, and insert quick assists and quick fixes
 * available at the current source viewer's quick assist invocation context.
 * <p>
 * The quick assist assistant can be configured with a {@link IQuickAssistProcessor} which provides the possible quick
 * assist and quick fix completions.
 * </p>
 * In order to provide backward compatibility for clients of <code>IQuickAssistAssistant</code>, extension interfaces
 * are used to provide a means of evolution. The following extension interfaces exist:
 * <ul>
 ...
 * </li> </p>
 * <p>
 * The interface can be implemented by clients. By default, clients use {@link QuickAssistAssistant} as the standard
...
public interface IQuickAssistAssistant
{
     ...
     * <p>
     * <strong>Note:</strong> This quick assist assistant will only be able to query the invocation context if
     * <code>sourceViewer</code> also implements {@link ISourceViewerExtension3}.
     * </p>
     * 
    ...
    /**
     * Registers a given quick assist processor for a particular content type. If there is already a processor
     * registered, the new processor is registered instead of the old one.
     * 
     * @param processor the quick assist processor to register, or <code>null</code> to remove an existing one
     */
     ...
     * <p>
     * <strong>Note:</strong> This test must be fast and optimistic i.e. it is OK to return <code>true</code> even
     * though there might be no quick fix.
     * </p>
     * 
    ...
    /**
     * Enables displaying a status line below the proposal popup. The default is not to show the status line. The
     * contents of the status line may be set via {@link #setStatusMessage(String)}.
     * 
