...
public class FormattingContext implements IFormattingContext
{
    ...
    /*
     * @see org.eclipse.jface.text.formatter.IFormattingContext#mapToStore(java.util.Map,
    ...
    /*
     * @see
     * org.eclipse.jface.text.formatter.IFormattingContext#storeToMap(org.eclipse.jface.preference.IPreferenceStore,
    ...
    public void storeToMap(IPreferenceStore store, Map map, boolean useDefault)
    {
        ...
        for (int index = 0; index < preferences.length; index++)
        {
            ...
            {
                map.put(preference,
                        (useDefault ? store.getDefaultBoolean(preference) : store.getBoolean(preference)) ? IPreferenceStore.TRUE
                                : IPreferenceStore.FALSE);
            }
            else if (isIntegerPreference(preference))
            {
                map.put(preference,
                        String.valueOf(useDefault ? store.getDefaultInt(preference) : store.getInt(preference)));
            }
            else if (isStringPreference(preference))
            {
            ...
            {
                map.put(preference,
                        String.valueOf(useDefault ? store.getDefaultDouble(preference) : store.getDouble(preference)));
            }
            else if (isFloatPreference(preference))
            {
                map.put(preference,
                        String.valueOf(useDefault ? store.getDefaultFloat(preference) : store.getFloat(preference)));
            }
            else if (isLongPreference(preference))
            {
                map.put(preference,
                        String.valueOf(useDefault ? store.getDefaultLong(preference) : store.getLong(preference)));
            }
        }
    }
}
