 ...
 * </ul>
 * The master formatting strategy always formats the whole region to be formatted in the first pass. In a second pass,
 * all partitions of the region to be formatted that are not of master content type are formatted using the slave
 * formatting strategy registered for the underlying content type. All formatting strategies must implement
 * {@link IFormattingStrategyExtension}.
 * <p>
 * Regions to be formatted with the master formatting strategy always have an offset aligned to the line start. Regions
 * to be formatted with slave formatting strategies are aligned on partition boundaries.
 * 
...
{
    /**
     * Position updater that shifts otherwise deleted positions to the next non-whitespace character. The length of the
     * positions are truncated to one if the position was shifted.
     */
    ...
    /*
     * @see org.eclipse.jface.text.formatter.IContentFormatterExtension#format(org.eclipse.jface.text.IDocument,
     ...
     * <p>
     * The master formatting strategy covers all regions of the document. The offset of the region to be formatted is
     * aligned on line start boundaries, whereas the end index of the region remains the same. For this formatting type
     * the document partitioning is not taken into account.
     * 
    ...
    /**
     * Formats the document specified in the formatting context with the formatting strategy registered for the content
     * type.
     * <p>
     * For this formatting type only slave strategies are used. The region to be formatted is aligned on partition
     * boundaries of the underlying content type. The exact formatting strategy is determined by the underlying content
     * type of the document partitioning.
     * 
     * @param context The formatting context to use
     * @param document The document to operate on
     * @param offset The offset of the region to format
     ...
     */
    protected void formatSlave(final IFormattingContext context, final IDocument document, final int offset,
            final int length, final String type)
    {
     ...
     * <p>
     * For each content type of the region to be formatted in the document partitioning, the registered slave formatting
     * strategy is used to format that particular region. The region to be formatted is aligned on partition boundaries
     * of the underlying content type. If the content type is the document's default content type, nothing happens.
     * 
     * @param context The formatting context to use
     * @param document The document to operate on
     * @param offset The offset of the region to format
     * @param length The length of the region to format
     */
    protected void formatSlaves(final IFormattingContext context, final IDocument document, final int offset,
            final int length)
    {
     ...
     * <p>
     * The strategy may already be registered with a certain content type as slave strategy. The master strategy is
     * registered for the default content type of documents. If a master strategy has already been registered, it is
     ...
     * <p>
     * The strategy may already be registered as master strategy. An already registered slave strategy for the specified
     * content type will be replaced. However, the same strategy may be registered with several content types. Slave
     * strategies cannot be registered for the default content type of documents.
     * 
