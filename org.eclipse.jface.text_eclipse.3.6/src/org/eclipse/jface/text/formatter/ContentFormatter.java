...
/**
 * Standard implementation of <code>IContentFormatter</code>. The formatter supports two operation modes: partition
 * aware and partition unaware.
 * <p>
 * In the partition aware mode, the formatter determines the partitioning of the document region to be formatted. For
 * each partition it determines all document positions which are affected when text changes are applied to the
 * partition. Those which overlap with the partition are remembered as character positions. These character positions
 * are passed over to the formatting strategy registered for the partition's content type. The formatting strategy
 * returns a string containing the formatted document partition as well as the adapted character positions. The
 * formatted partition replaces the old content of the partition. The remembered document positions are updated with the
 * adapted character positions. In addition, all other document positions are accordingly adapted to the formatting
 * changes.
 * <p>
 * In the partition unaware mode, the document's partitioning is ignored and the document is considered consisting of
 * only one partition of the content type <code>IDocument.DEFAULT_CONTENT_TYPE</code>. The formatting process is similar
 * to the partition aware mode, with the exception of having only one partition.
 * <p>
...
public class ContentFormatter implements IContentFormatter
{
    ...
    /**
     * The position updater which runs as first updater on the document's positions. Used to remove all affected
     * positions from their categories to avoid them from being regularly updated.
     * 
     * @see IPositionUpdater
     */
    ...
    /**
     * The position updater which runs as last updater on the document's positions. Used to update all affected
     * positions and adding them back to their original categories.
     * 
     * @see IPositionUpdater
     */
    ...
    /**
     * Creates a new content formatter. The content formatter operates by default in the partition-aware mode. There are
     * no preconfigured formatting strategies. Will use the default document partitioning if not further configured.
     */
    ...
    /**
     * Registers a strategy for a particular content type. If there is already a strategy registered for this type, the
     * new strategy is registered instead of the old one. If the given content type is <code>null</code> the given
     * strategy is registered for all content types as is called only once per formatting session.
     * 
    ...
    /**
     * Informs this content formatter about the names of those position categories which are used to manage the
     * document's partitioning information and thus should be ignored when this formatter updates positions.
     * 
     * @param categories the categories to be ignored
     * @deprecated incompatible with an open set of document partitionings. The provided information is only used if
     *             this formatter can not compute the partition managing position categories.
     */
    ...
    /**
     * Determines the partitioning of the given region of the document. Informs the formatting strategies of each
     * partition about the start, the process, and the termination of the formatting session.
     * 
     ...
     * about the start, the process, and the termination of the formatting session.
     * 
    ...
    /**
     * Returns the partitioning of the given region of the document to be formatted. As one partition after the other
     * will be formatted and formatting will probably change the length of the formatted partition, it must be kept
     * track of the modifications in order to submit the correct partition to all formatting strategies. For this, all
     * partitions are remembered as positions in a dedicated position category. (As formatting strategies might rely on
    ...
    /**
     * Fires <code>formatterStarts</code> to all formatter strategies which will be involved in the forthcoming
     * formatting process.
     * 
     ...
     * correctly, first all affected positions determined, after all document listeners have been informed about the
     * coming change, the affected positions are removed to avoid that they are regularly updated. After all position
     * updaters have run, the affected positions are updated with the formatter's information and added back to their
     * categories, right before the first document listener is informed about that a change happened.
     * 
    ...
    /**
     * Fires <code>formatterStops</code> to all formatter strategies which were involved in the formatting process which
    ...
    /**
     * Updates all the overlapping positions. Note, all other positions are automatically updated by their document
     * position updaters.
     * 
