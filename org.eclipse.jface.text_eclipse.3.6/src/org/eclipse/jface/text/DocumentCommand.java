 ...
 * {@link org.eclipse.swt.events.VerifyEvent} and translated into a document replace command relative to a given offset.
 * A document command can also be used to initialize a given <code>VerifyEvent</code>.
 * <p>
...
public class DocumentCommand
{
    ...
    /**
     * Fills the given verify event with the replace text and the <code>doit</code> flag of this document command.
     * Returns whether the document command covers the same range as the verify event considering the given offset.
     * 
    ...
    /**
     * Adds an additional replace command. The added replace command must not overlap with existing ones. If the
     * document command owner is not <code>null</code>, it will not get document change notifications for the particular
     * command.
     * 
    ...
    /**
     * Returns an iterator over the commands in ascending position order. The iterator includes the original document
     * command. Commands cannot be removed.
     * 
