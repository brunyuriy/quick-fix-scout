 ...
 * <p>
 * It monitors the text viewer and keeps a history of the changes applied to the viewer. The undo manager groups those
 * changes into user interactions which on an undo request are rolled back in one atomic change.
 * </p>
 * <p>
 * In order to provide backward compatibility for clients of <code>IUndoManager</code>, extension interfaces are used as
 * a means of evolution. The following extension interfaces exist:
 * <ul>
 * <li>{@link org.eclipse.jface.text.IUndoManagerExtension} since version 3.1 introducing access to the undo context.</li>
 * </ul>
 * </p>
 * <p>
 * Clients may implement this interface or use the standard implementation <code>TextViewerUndoManager</code>.
 * </p>
 * 
...
public interface IUndoManager
{
    ...
    /**
     * Disconnects this undo manager from its text viewer. If this undo manager hasn't been connected before this
     * operation has no effect.
     */
    void disconnect();

    /**
     * Signals the undo manager that all subsequent changes until <code>endCompoundChange</code> is called are to be
     * undone in one piece.
     */
    void beginCompoundChange();

    /**
     * Signals the undo manager that the sequence of changes which started with <code>beginCompoundChange</code> has
     * been finished. All subsequent changes are considered to be individually undo-able.
     */
    ...
    /**
     * Returns whether at least one text change can be repeated. A text change can be repeated only if it was executed
     * and rolled back.
     * 
